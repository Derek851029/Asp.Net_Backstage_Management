...
using System.Web.Script.Serialization;      // 002 多了 System.Web.UI.WebControls
...
public partial class _0030010099 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                Response.Redirect("/0030010000/0030010002.aspx");       //失敗下回10002
            }

            if (seqno != "0")
            {
                Session["Case_ID"] = seqno;
                sql_txt = @"SELECT TOP 1 SYS_ID FROM FROM [DimaxCallcenter].[dbo].[CaseData] WHERE Case_ID=@Case_ID";
                ...
                if (!chk.Any())
                {
                    ...
                    // 無10003.aspx?seqno=seqno 時自動跳回 10002
                }
                else
                {
                    ClassTemplate schedule = ClassScheduleRepository._0030010001_View_MNo(seqno);       // 執行_0030010001_View_MNo(seqno);
    ...
    public static string Load()
    {
        ...
        string Sqlstr = @"SELECT TOP 1 * FROM DispatchSystem WHERE Agent_ID = @Agent_ID AND Agent_Status != '離職'";
    ...
    {
        Check();
        string Sqlstr = @"SELECT DISTINCT BUSINESSNAME,ID FROM BusinessData ";
    ...
    {
        Check();
        string Sqlstr = @"SELECT BUSINESSNAME,APPNAME,APP_OTEL_AREA,APP_OTEL,APP_MTEL,HardWare,SoftwareLoad,SERVICEITEM FROM BusinessData WHERE ID=@value";
    ...
    public static string Case_Save(string sysid, string login_time, string client_id, string urgency, string op_source, string op_type, string esti_fin_time, string op_sub, string onspot_time, string opinion, string rep_type, string ps, string deal_process, string rep, int telecomm_id)
    {
        ...
        string Sqlstr = @"INSERT INTO CaseData (Case_ID,Upload_Time, ID, Urgency, EstimatedFinishTime, OpinionSource, OpinionType, OpinionSubject,OnSpotTime, OpinionContent, ReplyType, PS, DealingProcess, Reply, Telecomm_ID ) " +
           " VALUES( @sysid, @login_time, @client_id, @urgency, @esti_fin_time, @op_source, @op_type, @op_sub,@onspot_time,@opinion,@rep_type,@ps,@deal_process,@rep,@telecomm_id )";
        string status = "登單完成";
    ...
    {
        Check();
        string Sqlstr = @"SELECT DISTINCT Service FROM Data WHERE Open_Flag = '1' AND Service !='外包商' ";    //
        var a = DBTool.Query<ClassTemplate>(Sqlstr).ToList().Select(p => new
        {
            Service = p.Service
        });
    ...
    {
        Check();
        string Sqlstr = @"SELECT DISTINCT Cust_ID, Cust_FullName FROM CaseData WHERE Cust_ID != '' AND Labor_Type = '任用' ORDER BY Cust_FullName";
        var a = DBTool.Query<ClassTemplate>(Sqlstr).ToList().Select(p => new
        {
            Cust_ID = p.Cust_ID,
    ...
    {
        Check();
        string sqlstr = @"SELECT HospitalName FROM DataHospital WHERE Flag = '1'";
        var a = DBTool.Query<ClassTemplate>(sqlstr).ToList().Select(p => new
        {
    ...
    {
        Check();
        string Sqlstr = @"SELECT Service_ID, ServiceName FROM Data WHERE Open_Flag = '1' AND Service = @Service AND Service !='外包商' ";
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string GetCNoList()
    {
        Check();
        string Case_ID = HttpContext.Current.Session["Case_ID"].ToString();
        var a = PartnerHeaderRepository.CMS_0030010001_MNo_Labor(Case_ID)
            .Select(p => new
            {
                SYS_ID = p.SYS_ID,
    ...
    public static string Btn_Add_Click(string Cust_ID, string Labor_ID)
    {
        ...
        if (!string.IsNullOrEmpty(Cust_ID))
        {
            ...
            {
                sql_txt = @"SELECT TOP 1 SYS_ID FROM [DimaxCallcenter].[dbo].[CaseData]	 WHERE Case_ID=@Case_ID AND Labor_ID='' AND Cust_ID=@Cust_ID";
                var Labor_chk = DBTool.Query<String>(sql_txt, new { Case_ID = Case_ID, Labor_ID = Labor_ID, Cust_ID = Cust_ID });
                if (Labor_chk.Any())
                {
                ...
                {
                    sql_txt = @"SELECT TOP 1 SYS_ID FROM [DimaxCallcenter].[dbo].[CaseData]	 WHERE Case_ID=@Case_ID AND Labor_ID=@Labor_ID";
                    add_txt = @"INSERT INTO MNo_Labor " +
                   "(" +
                   "[Case_ID], [Cust_ID], [Cust_Name], [Cust_FullName], [Labor_ID], [Labor_CName], [Labor_EName], [Labor_Country], " +
            ...
            {
                sql_txt = @"SELECT TOP 1 SYS_ID FROM MNo_Labor WHERE Case_ID=@Case_ID AND Labor_ID != '' AND Cust_ID=@Cust_ID";
                var Labor_chk = DBTool.Query<String>(sql_txt, new { Case_ID = Case_ID, Labor_ID = Labor_ID, Cust_ID = Cust_ID });
                if (Labor_chk.Any())
                {
                    return JsonConvert.SerializeObject(new { status = back });
                ...
                {
                    sql_txt = @"SELECT TOP 1 SYS_ID FROM MNo_Labor WHERE Case_ID=@Case_ID AND Cust_ID=@Cust_ID";
                    add_txt = @"INSERT INTO MNo_Labor " +
                       "(" +
                       "[Case_ID], [Cust_ID], [Cust_Name], [Cust_FullName] " +
                       ") " +
                       " SELECT TOP 1 " +
            ...
            var chk = DBTool.Query<String>(sql_txt, new { Case_ID = Case_ID, Labor_ID = Labor_ID, Cust_ID = Cust_ID });
            if (chk.Any())
            {
            ...
            else
            {
                ...
                {
                    db.Execute(add_txt, new { Case_ID = Case_ID, Labor_ID = Labor_ID, Cust_ID = Cust_ID });
                    db.Close();
                }
    ...
    public static string Btn_Cancel_Click(string txt_cancel)
    {
        ...
        return JsonConvert.SerializeObject(new { status = "success", mno = Case_ID });
    }

    //=============【帶入需求單 母單內容】=============
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Load_MNo(string Case_ID)
    {
        Check();
        ...
        string sqlstr = @"SELECT TOP 1 * FROM LaborTemplate WHERE Case_ID=@Case_ID";
        string outputJson = "";
        var chk = DBTool.Query<ClassTemplate>(sqlstr, new { Case_ID = Case_ID });
        if (!chk.Any())
        {
            outputJson = "[" + JsonConvert.SerializeObject(new { Case_ID = "NULL" }) + "]"; // 組合JSON 格式
            return outputJson;
        }
        else
        {
            if (Agent_LV != "30")
            {
                foreach (var q in chk)
                {
                    ...
                    {
                        outputJson = "[" + JsonConvert.SerializeObject(new { Case_ID = "NO" }) + "]"; // 組合JSON 格式
                        return outputJson;
                    }
                }
            }
        ...
        string Hospital, string HospitalClass, string Question)
    {
        ...
        {
            sql_check = @"SELECT SYS_ID FROM [DimaxCallcenter].[dbo].[CaseData]	 WHERE Case_ID=@Case_ID";
            var chk = DBTool.Query<ClassTemplate>(sql_check, new { Case_ID = Case_ID });
            if (chk.Count() > 0)
            {
    ...
    {
        Check();
        string Sqlstr = @"SELECT SYS_ID, Name, Location, Postcode, Address, TEL FROM HRM2_Location WHERE Type = @Type ";        //  FROM
    ...
    public static string Add_Location(string ID, string Flag)
    {
        ...
        if (Flag == "0")
        {
            sqlstr = @"SELECT Name, Location, Postcode, Address FROM HRM2_Location WHERE SYS_ID = @SYS_ID ";        //  FROM
        ...
        {
            sqlstr = @"SELECT Cust_FullName, Cust_Postcode, Cust_Address_ZH FROM HRM2_Cust_System WHERE SYS_ID = @SYS_ID ";     //  FROM
            var a = DBTool.Query<Location_str>(sqlstr, new { SYS_ID = ID }).ToList().Select(p => new
            {
    ...
    public static string ServiceName_Flag(string Service_ID)
    {
        ...
        {
            string sql_txt = "";
            sql_txt = @"SELECT TOP 1 Flag_Add, Service FROM Data WHERE Service_ID=@Service_ID AND Open_Flag = '1' AND Service !='外包商' ";        //  FROM
