<%@ Page Language="C#" EnableEventValidation="false" AutoEventWireup="true" MasterPageFile="~/MasterPage.master" CodeFile="~/0030010005.aspx.cs" Inherits="_0030010005" %>
...
<asp:Content ID="Content" ContentPlaceHolderID="head2" runat="Server">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("[id$='Iframe2']").on('load', function () {
                //debugger;
                $("[id$='div_report2']").append($(this).contents().find("style"))
                                       .append($(this).contents().find("table"));
                $("[id$='div_report2'] table").addClass("table table-bordered table-striped");
            });
            document.getElementById('But_Delete').style.display = "none";  //隱藏刪除按鈕
            document.getElementById('But_ReCheck').style.display = "none";  //隱藏送出重審按鈕
            document.getElementById('Back_Table_0001').style.display = "none"; //退單原因
            document.getElementById('S_Enginner').style.display = "none";
            document.getElementById('S_Enginner2').style.display = "none";
            document.getElementById('Button9').style.display = "none";  //通知主管
            document.getElementById('Button10').style.display = "none"; //派工
            Load();
            Dealer();
...
/*        function ALL() {
        ...
        function Load() {            
            if (seqno == '0') {
                ...
                document.getElementById('Button4').style.display = "none";  //隱藏結案按鈕
                        ...
                        columns: [
                                ...
                                ///*{ data: "Price" },
                                {
                                    data: "ID", render: function (data, type, row, meta) {
                                        return "<input type='int' id='price_" + data + "' class='form-control' style=' width: 150px;' />";
                                    }   //onkeyup='this.value=this.value.replace(/\D/g,'')' onafterpaste='this.value=this.value.replace(/\D/g,'')'  //onkeyup沒作用
                                },  //*/
                                {
                                data: "ID", render: function (data, type, row, meta) {
                                    return "<input type='int' id='txt_" + data + "' class='form-control' style=' width: 50px;' " +
                                        " maxlength='5' onkeyup='value=value.replace(/[^\d]/g,'')'/>";
                                    }
                                },
                                {
                                data: "", render: function (data, type, row, meta) {
                                    return "<button type='button' class='btn btn-success btn-lg' data-toggle='modal' " +
                                        " style='Font-Size: 20px;' id='edit' class='btn btn-info btn-lg'>" + //data-target='#newModal'
...
//document.getElementById("str_com_tel").innerHTML = "("+ obj.data[0].E + ")" + obj.data[0].F;
 ...
 /*       function ShowCaseData() {
        ...
        function Data_Save() {                  //確定新增 OE 單            
            ...
            var Type_Value = document.getElementById("txt_Type_Value").value; //OE單號
        ...
        function Data_Update(Flag) {                  //右方 存入 左方
            ...
            var str_sysid = document.getElementById("str_sysid").innerHTML;
            var oe_id = document.getElementById("OE_D_ID").innerHTML;
            var OE_PS = document.getElementById("Textarea1").value;
            var Con_Name = document.getElementById("Text1").value;
            var Con_Company = document.getElementById("Label2").innerHTML;
            var Con_Phone = document.getElementById("Text2").value;
            ...
            var enginner_ID = document.getElementById("Lab_Enginner_ID").innerHTML;
            var enginner_Name = document.getElementById("Lab_Enginner_Name").innerHTML;

            $.ajax({
                url: '0030010005.aspx/Case_Save2',
                type: 'POST',
                data: JSON.stringify({
                    Flag: Flag, // 0修改 1結案 2重新送審 3通知工程主管 4已派工
                    sysid: str_sysid,
                    OE_PS: OE_PS,
                    Con_Name: Con_Name,
                    Con_Company: Con_Company,
                ...
                success: function (doc) {
                    ...
                    else if (json.status == "recheck") {
                        alert('已重新送審');
                        window.location.href = "/0030010005.aspx?seqno="+str_sysid;
                    }
                    else if (json.status == "dispatch") {
                        alert('已通知工程主管');
                        window.location.href = "/0030010005.aspx?seqno=" + str_sysid;
                    }
                    else if (json.status == "dispatched") {
                        alert('已通知工程師');
                        window.location.href = "/0030010005.aspx?seqno=" + str_sysid;
                    }
                    else {
...
/*        function Load_MNo() {
       ...
       //================   Load_CaseData()       
        function Load_OE_Data() {          
            ...
            document.getElementById('Button1').style.display = "none";  //隱藏新增
            document.getElementById('Button2').style.display = "";  //顯示 修改按鈕
            ...
            $.ajax({
                ...
                success: function (doc) {
                    ...
                    document.getElementById("Back").innerHTML = obj.data[0].Chargeback;
                    document.getElementById("Lab_Enginner_Name").innerHTML = obj.data[0].Enginner;
                    document.getElementById("Lab_Enginner_ID").innerHTML = obj.data[0].Enginner_ID;

                    document.getElementById("OE_D_ID").innerHTML = obj.data[0].OE_D_ID;

                    if (obj.data[0].Type == "尚未審查") {
                        if (obj.data[0].Con_Name == obj.data[0].Agent_Name) //建單人
                            //alert("建單人");
                            document.getElementById('But_Delete').style.display = "";   //顯示刪除紐
                        else if (obj.data[0].Con_Company == obj.data[0].Agent_Company && obj.data[0].Agent_Team == "Leader") //部門的主管
                            //alert(obj.data[0].Con_Company + " Leader");
                            document.getElementById('But_Delete').style.display = "";   //顯示刪除紐
                        else if (obj.data[0].Con_Company == "Sales 3" && obj.data[0].Agent_Name == "紀守仁") //部門的主管
                            //alert("紀守仁");
                            document.getElementById('But_Delete').style.display = "";   //顯示刪除紐
                        else if ((obj.data[0].Con_Company == "MA_Sales" || obj.data[0].Con_Company == "Engineer") && obj.data[0].Agent_Name == "陳仁杰") //部門的主管
                            //alert("陳仁杰");
                            document.getElementById('But_Delete').style.display = "";   //顯示刪除紐
                    }
                    else if (obj.data[0].Type == "退單") {
                        document.getElementById("Back_Table_0001").style.display = "";
                        if (obj.data[0].Con_Name == obj.data[0].Agent_Name) //建單人
                        {
                            document.getElementById('But_Delete').style.display = "";
                            document.getElementById('But_ReCheck').style.display = "";//送出重審鈕
                        }
                    }
                    else if (obj.data[0].Type == "已經審查") {
                        document.getElementById('Button9').style.display = "";  //
                    }
                    else if (obj.data[0].Type == "主管派工階段") {
                        document.getElementById('S_Enginner').style.display = "";   //顯示工程師選項
                        document.getElementById('S_Enginner2').style.display = "";   //顯示工程師選項
                        document.getElementById('Button10').style.display = "";  //
                    }
                    else if (obj.data[0].Type == "已派工") {
                        document.getElementById('Button4').style.display = "";  //結案鈕
                    }

                    //alert("T=" + obj.data[0].Type + "  C_N=" + obj.data[0].Con_Name + "  C_C=" + obj.data[0].Con_Company);
                    //alert("A_N=" + obj.data[0].Agent_Name + "  A_C=" + obj.data[0].Agent_Company + "  A_T=" + obj.data[0].Agent_Team);
                }
            });
        ...
        function OE_Delete() {
            var str_sysid = document.getElementById("OE_D_ID").innerHTML;
            if (confirm("確定要刪除OE單嗎？")) {
                $.ajax({
                    url: '0030010005.aspx/Delete_OE',
                    ache: false,
                    type: 'POST',
                    //async: false,
                    data: JSON.stringify({ sysid: str_sysid }),
                    contentType: 'application/json; charset=UTF-8',
                    dataType: "json",       //如果要回傳值，請設成 json
                    success: function (data) {
                        var json = JSON.parse(data.d.toString());
        ...
        function Change_Dispatch() {
            $.ajax({
                url: '0030010005.aspx/List_Dispatch_Name',
                type: 'POST',
                contentType: 'application/json; charset=UTF-8',
                dataType: "json",
                success: function (doc) {
        ...
        function Load_Enginner() {
            var A_ID = document.getElementById("Dispatch_Name").value;
            if (A_ID == "") {
                document.getElementById("Lab_Enginner_Name").innerHTML = "";
                document.getElementById("Lab_Enginner_ID").innerHTML = "";
            }
            $.ajax({
                url: '0030010005.aspx/Load_Agent',
                type: 'POST',
                data: JSON.stringify({ Agent_ID: A_ID }),
                contentType: 'application/json; charset=UTF-8',
                dataType: "json",
                success: function (doc) {
                    var text = '{"data":' + doc.d + '}';
                    var obj = JSON.parse(text);
                    document.getElementById("Lab_Enginner_Name").innerHTML = obj.data[0].Agent_Name;
                    document.getElementById("Lab_Enginner_ID").innerHTML = obj.data[0].Agent_ID;
                }
            });
        }
            ...
            <thead>
                <tr>
                    <th style="text-align: center" colspan="4">
                        ...
                        <label id="OE_D_ID" hidden="hidden"></label>
                    </th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    ...
                    <td>
                        <div style="float: left" data-toggle="tooltip" title="不能超過 500 個字元">      
                                <textarea id="Textarea1" name="OpinionSubject" class="form-control" cols="45" rows="3" placeholder="建單備註" maxlength="500" onkeyup=""
...
<%--  =========== 表單四 OE 商品列表===========--%>
       ...
       <table  id="data" class="display table table-striped" style="width: 99%">            
            <thead>
                <tr>
                    ...
                    <th style="text-align: center;" width: auto;>商品名稱</th>
                    <th style="text-align: center;" width: auto;>主分類</th>
                    <th style="text-align: center;" width: auto;>子分類</th>
                    <th style="text-align: center;" width: auto;>編號</th>
                    <th style="text-align: center;" width: 8%;>價格</th>                    
    ...
    <div class="table-responsive" style="width: 90%; margin: 10px 20px">
       <table  id="data2" class="display table table-striped" style="width: 99%">            
            <thead>
                <tr>
                    <th style="text-align: center;" width: auto;>ID</th>                    
                    <th style="text-align: center;" width: auto;>OE單號</th>
                    <th style="text-align: center;" width: auto;>商品名稱</th> 
                    <th style="text-align: center;" width: auto;>編號</th>                  
                    <th style="text-align: center;" width: auto;>數量</th>
                    <th style="text-align: center;" width: auto;>單價</th>
                    <th style="text-align: center;" width: auto;>合計</th>
...
<%--  =========== 表單六 =開關               ========--%>
                ...
                <tr id="Tr_upload2">
                    ...
                    <th>
                        <button id="Button8" type="button" class="btn btn-info btn-lg" data-toggle="modal" style="Font-Size: 20px;" onclick="URL4();">
                            <span class='glyphicon glyphicon-search'></span>&nbsp;&nbsp;檔案清單</button>
                    </th>
                    <th style="text-align: center;">定單下載</th>
                    <th>
                        <asp:Button ID="Btn_exportExcel" runat="server" Text="有價格" Width="100px" TabIndex="-1" Enabled="true" type="button" class="btn btn-success" OnClick="Btn_exportExcel_Click" />                        
                        <asp:Button ID="Btn_exportExcel2" runat="server" Text="無價格" Width="100px" TabIndex="-1" Enabled="true" type="button" class="btn btn-success" OnClick="Btn_exportExcel_Click2" />
                    </th>
                </tr>
                <tr>
                    <td colspan="4">                        
                        <iframe id="Iframe1" runat="server" style="display: none;"></iframe>
                        <iframe id="Iframe2" runat="server" style="display: none;"></iframe>
                        <!--    -->
                    </td>
                </tr>
            </tbody>
    ...
    <%--  =========== 報表   刪除也能下載            ========--%>
    <div style="width: 1280px; margin: 10px 20px" hidden="hidden">
        <h2 id="report_title" runat="server"><strong></strong></h2>
        <div id="div_report"></div>
        <div id="div_report2"></div>
    </div>      
    <%--  =========== 表單六 其他資料               ========--%>
        <div style="text-align: center; width: 1280px; margin: 10px 20px">
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <th style="text-align: center; width: 15%; height: 55px;">
                            <strong>工程師</strong><br />
                            <label id="Lab_Enginner_ID"></label>
                        </th>
                        <th>
                            <label id="Lab_Enginner_Name"></label>
                        </th>
                        <th id="S_Enginner" style="text-align: center; width: 15%; height: 55px;">
                            <strong>選擇工程師</strong>
                        </th>
                        <th id="S_Enginner2">
                            <select id="Dispatch_Name" name="Dispatch_Name" class="chosen-select" onchange="Load_Enginner();">
                            <option value="">請選擇派工人員…</option>
                        </select>
                        </th>
                    </tr>        
                    <tr id="Back_Table_0001"  style="height: 80px;">
                        <td style="text-align: center; color: #D50000;">
                            <strong>退單原因</strong>
                        </td>
                        <td style="text-align: center;"  colspan="3">
                            <div style="float: left" data-toggle="tooltip" title="100字以內">
                                <label id="Back"></label>
                            </div>
                        </td>
                    </tr> 
    ...
    <div style="text-align: center">
        ...
        <button id="But_ReCheck" type="button" onclick="Data_Update('2');" class="btn btn-success btn-lg "><span class="glyphicon glyphicon-file"></span>&nbsp;&nbsp;重新送審</button>&nbsp; &nbsp;
        <button id="Button9" type="button" onclick="Data_Update('3');" class="btn btn-success btn-lg "><span class="glyphicon glyphicon-envelope"></span>&nbsp;&nbsp;通知工程主管</button>&nbsp; &nbsp;
        <button id="Button10" type="button" onclick="Data_Update('4');" class="btn btn-success btn-lg "><span class="glyphicon glyphicon-envelope"></span>&nbsp;&nbsp;OE派工</button>&nbsp; &nbsp;
        ...
        <button id="But_Delete" type="button" onclick="OE_Delete();" class="btn btn-danger btn-lg"><span class="glyphicon glyphicon-remove"></span>&nbsp;&nbsp;刪除</button>&nbsp; &nbsp;       
    </div>
        <!--===================================================-->
        <script>
