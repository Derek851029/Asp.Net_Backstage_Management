...
/*                    if (schedule.Type_Value == "1")
    ...
    public static string Dealer()                   // 出現在 21, 208 行
    {
        ...
        var a = DBTool.Query<T_0030010005_2>(Sqlstr).ToList().Select(p => new
        {
            A = Value(p.D_Name),
            B = p.ID,
            C = p.PID
    ...
    public static string OE_List(string PID)                                                       // 新增 讀BusinessData 案件資料
    {
        ...
        {
            sqlstr = @"SELECT * FROM OE_Data  WHERE PID=@PID";
        }    //string outputJson = "";
        else if (Agent_Company1 == "Engineer" && Agent_Team == "Leader")
            sqlstr = @"SELECT * FROM OE_Data  WHERE PID=@PID and Con_Company in( 'Engineer', 'MA_Sales')";
        else
            sqlstr = @"SELECT * FROM OE_Data  WHERE PID=@PID and Con_Company = @Agent_Company";

        var lcd = DBTool.Query<T_0030010005_2>(sqlstr, new { PID = PID, Agent_Company = Agent_Company1 }).ToList().Select(p => new
            {
        ...
        //string outputJson = "";
        var lcd2 = DBTool.Query<T_0030010005_2>(sqlstr2, new { PID = PID,Agent_Company2= Agent_Company1 }).ToList().Select(p => new
        {
            OE_Case_ID = Value(p.OE_Case_ID),
            BUSINESSNAME = Value(p.BUSINESSNAME),
            Name = Value(p.Name),
    ...
    public static string URL4(string mno)
    {
        ...
        if (JASON.IsInt(mno) != true)
        {
            return JsonConvert.SerializeObject(new { status = error + "_1  " + mno });
        }

        if (mno.Length > 16 || mno.Length < 1)
        {
            return JsonConvert.SerializeObject(new { status = error + "_2 序號長度錯誤" });
        }
        ...
        //string str_url = "http://118.163.27.22:5888/OE_File/" + mno + (char)92;         // 極緻用
        string str_url = "http://123.51.218.13:5888/OE_File/" + mno + (char)92;         // 德瑪用
        return JsonConvert.SerializeObject(new { status = str_url, type = "ok" });
    }

    [WebMethod(EnableSession = true)]//或[WebMethod(true)]       //OE商品列表
    public static string GetOEList(string Main, string Detail)       //案件列表程式   
    {
        string sqlstr2 = "";
        if (Main == "all" && Detail == "all")
            sqlstr2 = @" select * From OE_Product ";
        else if (Main == "all")
            sqlstr2 = @" select * From OE_Product where Detail_Classified = @Detail ";
        else if (Detail == "all")
            sqlstr2 = @" select * From OE_Product where Main_Classified = @Main ";
        else
            sqlstr2 = @" select * From OE_Product where Main_Classified=@Main and Detail_Classified = @Detail ";
    ...
    {
        //Check();
        OE_O_ID = Value(OE_O_ID);
        string sqlstr = @"Update OE_Order set Delete_Date = getdate() WHERE OE_O_ID = @OE_O_ID";
        DBTool.Query<T_0030010005_2>(sqlstr, new { OE_O_ID = OE_O_ID });  //, UpdateTime = DateTime.Now
    ...
    {
        OE_Case_ID = Value(OE_Case_ID);
        string sqlstr2 = @" Select a.*, Product_ID From OE_Order a
            Left Join OE_Product b on a.OE_ID=b.OE_ID 
            Where OE_Case_ID = @OE_Case_ID and Delete_Date is null ";
        var a = DBTool.Query<T_0030010005_2>(sqlstr2, new
        {
    ...
    public static string OE_Total_Price(string OE_Case_ID)
    {
        //Check();
        OE_Case_ID = Value(OE_Case_ID);
        string sqlstr = @"SELECT SUM([Unit_Price]*[Quantity]) as Total_Price FROM OE_Order where OE_Case_ID = @OE_Case_ID and Delete_Date is null ";
        var a = DBTool.Query<T_0030010005_2>(sqlstr, new { OE_Case_ID = OE_Case_ID }).ToList().Select(p => new
        {
...
/*    [WebMethod(EnableSession = true)]//或[WebMethod(true)]           訂貨儲存
    public static int ALL(string ID)
    {
        ...
        var Team_Array = DBTool.Query<T_0030010005_2>(sqlstr, new { OE_Case_ID = ID });
 ...
 //       }
        ...
        var a = DBTool.Query<T_0030010005_2>(Sqlstr, new
        {
            OE_Case_ID = OE_Case_ID,
            OE_ID = ID,
            Quantity = Main,
    ...
    public static string Show_Client_Data(string value)                     // 叫出商業資料  234行
    {
        ...
        var a = DBTool.Query<T_0030010005_2>(Sqlstr, new { value = value }).ToList().Select(p => new
        {
            A = Value(p.PID),
            B = Value(p.ID),
            C = Value(p.BUSINESSNAME),         // 公司名
            D = Value(p.APPNAME),                  // 聯絡人
            E = Value(p.CONTACT_ADDR),
            F = Value(p.APP_FTEL),
            G = Value(p.APP_MTEL),                 // 手機
            H = Value(p.HardWare),      // 硬體
    ...
    public static string Show_Client_Data2(string value_2)                     // 叫出商業資料  234行
    {
        ...
        var a = DBTool.Query<T_0030010005_2>(Sqlstr, new { value_2 = value_2 }).ToList().Select(p => new
        {
            B = p.ID,
            C = p.Name,         // 公司名
            D = p.APPNAME,                  // 聯絡人
    ...
    // 新單子 數據登錄
    {
        ...
        if (JASON.IsNumeric(Final_Price) != true)
        {
            return JsonConvert.SerializeObject(new { status = "【最終總價】只能輸入數字。" });
        }

        string back = "";
        List<XXS> check_value = new List<XXS>();
        check_value.Add(new XXS { URL_ID = OE_PS, MiniLen = 0, MaxLen = 500, Alert_Name = "建單備註", URL_Type = "txt" });
        check_value.Add(new XXS { URL_ID = Con_Name, MiniLen = 0, MaxLen = 25, Alert_Name = "承辦人員", URL_Type = "txt" });
        check_value.Add(new XXS { URL_ID = Con_Phone, MiniLen = 0, MaxLen = 20, Alert_Name = "連絡電話", URL_Type = "txt" });
        check_value.Add(new XXS { URL_ID = Type_Value, MiniLen = 0, MaxLen = 10, Alert_Name = "OE單號", URL_Type = "txt" });
        check_value.Add(new XXS { URL_ID = APPNAME, MiniLen = 0, MaxLen = 25, Alert_Name = "客戶聯絡人", URL_Type = "txt" });
        check_value.Add(new XXS { URL_ID = CONTACT_ADDR, MiniLen = 0, MaxLen = 150, Alert_Name = "客戶地址", URL_Type = "txt" });
        check_value.Add(new XXS { URL_ID = APP_FTEL, MiniLen = 0, MaxLen = 25, Alert_Name = "客戶傳真電話", URL_Type = "txt" });
        check_value.Add(new XXS { URL_ID = APP_MTEL, MiniLen = 0, MaxLen = 25, Alert_Name = "客戶聯絡電話", URL_Type = "txt" });
        check_value.Add(new XXS { URL_ID = APP_FTEL, MiniLen = 0, MaxLen = 100, Alert_Name = "硬體", URL_Type = "txt" });
        check_value.Add(new XXS { URL_ID = APP_MTEL, MiniLen = 0, MaxLen = 100, Alert_Name = "軟體", URL_Type = "txt" });
        ...
        if (!string.IsNullOrEmpty(Warranty_Date))
            if (!DateTime.TryParse(Warranty_Date, out Date))
                return JsonConvert.SerializeObject(new { status = "保固開始時間錯誤 請修正後再新增OE" });
        if (!string.IsNullOrEmpty(Warr_Time))
            if (!DateTime.TryParse(Warr_Time, out Date))
                return JsonConvert.SerializeObject(new { status = "保固結束時間錯誤 請修正後再新增OE" });
        if (!string.IsNullOrEmpty(Protect_Date))
            if (!DateTime.TryParse(Protect_Date, out Date))
                return JsonConvert.SerializeObject(new { status = "維護開始時間錯誤 請修正後再新增OE" });
        if (!string.IsNullOrEmpty(Prot_Time))
            if (!DateTime.TryParse(Prot_Time, out Date))
                return JsonConvert.SerializeObject(new { status = "維護結束時間錯誤 請修正後再新增OE" });
        ...
        string status = "new";
        a = DBTool.Query<T_0030010005_2>(Sqlstr, new
        {
            OE_Case_ID = sysid,
            OE_PS = OE_PS,
            Con_Name = Con_Name,
        ...
        if (!string.IsNullOrEmpty(Warranty_Date))   // 當 保固開始 Warranty_Date 非null或Empty時 update
        {
            Sqlstr = @"UPDATE OE_Data SET Warranty_Date = @Warranty_Date " +
                " Where OE_Case_ID = @OE_Case_ID " +
                " UPDATE BusinessData Set Warranty_Date = @Warranty_Date Where PID=@PID ";
            a = DBTool.Query<T_0030010005_2>(Sqlstr, new
            { OE_Case_ID = sysid,Warranty_Date = Test(Warranty_Date),PID = PID });            
        }
        else    //將 Warranty_Date 改成 null
        {
            Sqlstr = @"UPDATE OE_Data SET Warranty_Date = null where OE_Case_ID = @OE_Case_ID ";
            a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid });
        }

        if (!string.IsNullOrEmpty(Warr_Time))   // 當 保固開始 Warr_Time 非null或Empty時 update
        {
            Sqlstr = @"UPDATE OE_Data SET Warr_Time = @Warr_Time Where OE_Case_ID = @OE_Case_ID " +
                " UPDATE BusinessData Set Warr_Time = @Warr_Time Where PID=@PID " +
                //" If Exists( Select PID,Warr_Time,Warr_Mail From BusinessData Where Month(Warr_Time)<Month(getdate())+2 and PID=@PID ) " +
                " If Exists( Select PID,Warr_Time,Warr_Mail From BusinessData Where Warr_Time<getdate()+62 and PID=@PID ) " +
                " Update BusinessData Set Warr_Mail='1' where PID=@PID " +
                " Else Update BusinessData Set Warr_Mail='0' where PID=@PID ";
            a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid, Warr_Time = Test(Warr_Time), PID = PID });
        }
        else    //將 Warr_Time 改成 null
        {
            Sqlstr = @"UPDATE OE_Data SET Warr_Time = null where OE_Case_ID = @OE_Case_ID ";
            a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid });
        }

        if (!string.IsNullOrEmpty(Protect_Date))   // 當 保固開始 Protect_Date 非null或Empty時 update
        {
            Sqlstr = @"UPDATE OE_Data SET Protect_Date = @Protect_Date " +
                " Where OE_Case_ID = @OE_Case_ID " +
                " UPDATE BusinessData Set Protect_Date = @Protect_Date Where PID=@PID ";
            a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid, Protect_Date = Test(Protect_Date), PID = PID });
        }
        else    //將 Protect_Date 改成 null
        {
            Sqlstr = @"UPDATE OE_Data SET Protect_Date = null where OE_Case_ID = @OE_Case_ID ";
            a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid });
        }

        if (!string.IsNullOrEmpty(Prot_Time))   // 當 保固開始 Prot_Time 非null或Empty時 update
        {
            Sqlstr = @"UPDATE OE_Data SET Prot_Time = @Prot_Time Where OE_Case_ID = @OE_Case_ID " +
                " UPDATE BusinessData Set Prot_Time = @Prot_Time Where PID=@PID " +
                //" If Exists( Select PID,Prot_Time,Prot_Mail From BusinessData Where Month(Prot_Time)<Month(getdate())+2 and PID=@PID ) " +
                " If Exists( Select PID,Prot_Time,Prot_Mail From BusinessData Where Prot_Time<getdate()+62 and PID=@PID ) " +
                " Update BusinessData Set Prot_Mail='1' where PID=@PID " +
                " Else Update BusinessData Set Prot_Mail='0' where PID=@PID ";
            a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid, Prot_Time = Test(Prot_Time), PID = PID });
        }
        else    //將 Prot_Time 改成 null
        {
            Sqlstr = @"UPDATE OE_Data SET Prot_Time = null where OE_Case_ID = @OE_Case_ID ";
            a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid });
        }
        ...
        if (Agent_Company == "Sales 1" || Agent_Company == "Sales1" || Agent_Company == "Sales_1")
            sqlstr2 = @"SELECT Top 1 Agent_Name, Agent_Mail, UserID FROM DispatchSystem where Agent_Status != '離職' and Agent_Company in('Sales 1','Sales1','Sales_1') and Agent_Team ='Leader'  ";
        else if (Agent_Company == "Sales 2" || Agent_Company == "Sales2" || Agent_Company == "Sales_2")
            sqlstr2 = @"SELECT Top 1 Agent_Name, Agent_Mail, UserID FROM DispatchSystem where Agent_Status != '離職' and Agent_Company in('Sales 2','Sales2','Sales_2') and Agent_Team ='Leader'  ";
        else if (Agent_Company == "Sales 3" || Agent_Company == "Sales3" || Agent_Company == "Sales_3")
            sqlstr2 = @"SELECT Top 1 Agent_Name, Agent_Mail, UserID FROM DispatchSystem where Agent_Status != '離職' and Agent_Company in('Sales 3','Sales3','Sales_3') and Agent_Team ='Leader' "+
                " or Agent_Name='紀守仁' Order by Agent_Company ";
            //sqlstr2 = @"SELECT Top 1 Agent_Name, Agent_Mail, UserID FROM DispatchSystem where Agent_Status != '離職' and ( (Agent_Company in('Sales 3','Sales3','Sales_3') and Agent_Team ='Leader') "+
            //    " or Agent_Name='紀守仁' ) Order by Agent_Company "; //好像這才對?
        else if (Agent_Company == "Sales 4" || Agent_Company == "Sales4" || Agent_Company == "Sales_4")
            sqlstr2 = @"SELECT Top 1 Agent_Name, Agent_Mail, UserID FROM DispatchSystem where Agent_Status != '離職' and Agent_Company in('Sales 4','Sales4','Sales_4') and Agent_Team ='Leader'  ";
        else if (Agent_Company == "MA_Sales" || Agent_Company == "Engineer")
            sqlstr2 = @"SELECT Top 1 Agent_Name, Agent_Mail, UserID FROM DispatchSystem where Agent_Status != '離職' and Agent_Company='Engineer' and Agent_Team ='Leader' Order by Agent_Name ";
        //通知 陳仁杰 而不是簡子翔 以名稱排序 通知第一位 Engineer Leader */
        else
            return JsonConvert.SerializeObject(new { status = status });    //非條件部門不通知
        //Sales 3 通知    2    紀守仁	Pre-sale (Pre-sale 目前三人)
        //Sales_4 通知    36  呂子斌	Sales_4  Leader
        //MA_Sales Engineer 通知    	Engineer Leader
        var list = DBTool.Query<ClassTemplate>(sqlstr2 );
        if (list.Count() > 0)
        {
    ...
    public static string Case_Save2(string Flag, string sysid, string OE_PS, string Con_Name, string Con_Company, string Con_Phone, //string SetupDate,
        ...
        string Final_Price, string Warranty_Date, string Warr_Time, string Protect_Date, string Prot_Time,
        string Receipt_Date, string Receipt_PS, string Close_Out_Date, string Close_Out_PS, string OE_ID, string Back,
        string Enginner_ID, string Enginner_Name)
    // 新單子 數據登錄
    {
        ...
        if (!string.IsNullOrEmpty(Warranty_Date))
            if (!DateTime.TryParse(Warranty_Date, out Date))
                return JsonConvert.SerializeObject(new { status = "保固開始時間錯誤 請修正後再儲存" });
        if (!string.IsNullOrEmpty(Warr_Time))
            if (!DateTime.TryParse(Warr_Time, out Date))
                return JsonConvert.SerializeObject(new { status = "保固結束時間錯誤 請修正後再儲存" });
        if (!string.IsNullOrEmpty(Protect_Date))
            if (!DateTime.TryParse(Protect_Date, out Date))
                return JsonConvert.SerializeObject(new { status = "維護開始時間錯誤 請修正後再儲存" });
        if (!string.IsNullOrEmpty(Prot_Time))
            if (!DateTime.TryParse(Prot_Time, out Date))
                return JsonConvert.SerializeObject(new { status = "維護結束時間錯誤 請修正後再儲存" });
        ...
        else if (Flag == "4")
        {
            Type = "已派工";
            status = "dispatched";
            if (Enginner_ID == "" || Enginner_Name == "")
                return JsonConvert.SerializeObject(new { status = "請選擇派工人員" });
        }
        //DateTime time = DateTime.Parse(login_time);
        ...
        try
        {
            string Sqlstr = @"UPDATE OE_Data SET " +
                ...
                "WHERE OE_Case_ID = @OE_Case_ID and OE_D_ID=@OE_ID";
            //status = "update";
            var a = DBTool.Query<T_0030010005_2>(Sqlstr, new
            {
                OE_Case_ID = sysid,
            ...
            if (!string.IsNullOrEmpty(Warranty_Date))   // 當 保固開始 Warranty_Date 非null或Empty時 update
            {
                Sqlstr = @"If Exists( Select Warranty_Date From OE_Data Where OE_Case_ID = @OE_Case_ID and " +
                    " (Warranty_Date is null or  Warranty_Date!=@Warranty_Date) )  " +
                    " UPDATE BusinessData Set Warranty_Date = @Warranty_Date Where PID=@PID " +
                    " UPDATE OE_Data SET Warranty_Date = @Warranty_Date Where OE_Case_ID = @OE_Case_ID ";
                a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid, Warranty_Date = Test(Warranty_Date), PID = PID });
            }
            else    //將 Warranty_Date 改成 null
            {
                Sqlstr = @"UPDATE OE_Data SET Warranty_Date = null where OE_Case_ID = @OE_Case_ID ";
            ...
            if (!string.IsNullOrEmpty(Warr_Time))   // 當 保固開始 Warr_Time 非null或Empty時 update
            {
                Sqlstr = @"If Exists( Select Warr_Time From OE_Data Where  OE_Case_ID = @OE_Case_ID and " +
                    " (Warr_Time is null or Warr_Time!=@Warr_Time) )" +
                    " BEGIN UPDATE BusinessData Set Warr_Time = @Warr_Time Where PID=@PID " +
                    //" If Exists( Select PID,Warr_Time,Warr_Mail From BusinessData Where Month(Warr_Time)<Month(getdate())+2 and PID=@PID ) " +
                    " If Exists( Select PID,Warr_Time,Warr_Mail From BusinessData Where Warr_Time)<getdate()+62 and PID=@PID ) " +
                    " Update BusinessData Set Warr_Mail='1' where PID=@PID " +
                    " Else Update BusinessData Set Warr_Mail='0' where PID=@PID END" +
                    " UPDATE OE_Data SET Warr_Time = @Warr_Time Where OE_Case_ID = @OE_Case_ID ";
                a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid, Warr_Time = Test(Warr_Time), PID = PID });
            }
            else    //將 Warr_Time 改成 null
            {
            ...
            if (!string.IsNullOrEmpty(Protect_Date))   // 當 維護開始 Protect_Date 非null或Empty時 update
            {
                Sqlstr = @"If Exists( Select Protect_Date From OE_Data Where OE_Case_ID = @OE_Case_ID and " +
                    " (Protect_Date is null or  Protect_Date!=@Protect_Date) )  " +
                    " UPDATE BusinessData Set Protect_Date = @Protect_Date Where PID=@PID " +
                    " UPDATE OE_Data SET Protect_Date = @Protect_Date Where OE_Case_ID = @OE_Case_ID ";
                a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid, Protect_Date = Test(Protect_Date), PID = PID });
            }
            else    //將 Protect_Date 改成 null
            {
            ...
            if (!string.IsNullOrEmpty(Prot_Time))   // 當 維護開始 Prot_Time 非null或Empty時 update
            {
                Sqlstr = @"If Exists( Select Prot_Time From OE_Data Where  OE_Case_ID = @OE_Case_ID and " +
                    " (Prot_Time is null or Prot_Time!=@Prot_Time) )" +
                    " BEGIN UPDATE BusinessData Set Prot_Time = @Prot_Time Where PID=@PID " +
                    //" If Exists( Select PID,Prot_Time,Prot_Mail From BusinessData Where Month(Prot_Time)<Month(getdate())+2 and PID=@PID ) " +
                    " If Exists( Select PID,Prot_Time,Prot_Mail From BusinessData Where Prot_Time<getdate()+62 and PID=@PID ) " +
                    " Update BusinessData Set Prot_Mail='1' where PID=@PID " +
                    " Else Update BusinessData Set Prot_Mail='0' where PID=@PID END" +
                    " UPDATE OE_Data SET Prot_Time = @Prot_Time Where OE_Case_ID = @OE_Case_ID ";
                a = DBTool.Query<T_0030010005_2>(Sqlstr, new { OE_Case_ID = sysid, Prot_Time = Test(Prot_Time), PID = PID });
            }
            else    //將 Prot_Time 改成 null
            {
            ...
            if (Flag == "2")
            {
                ...
                if (list.Count() > 0)
                {
                    ...
                    foreach (var q in list)
                    {
                        ClassTemplate emma = new ClassTemplate()
                        {
                            AssignNo = "【OE重新審核】【" + BUSINESSNAME + "】",   // + q.Agent_Name 
                            ConnURL = EMMA + JASON.Encryption(q.UserID),
                            //ConnURL = EMMA + JASON.Encryption("admin"),   //測試用
                            E_MAIL = q.Agent_Mail,
            ...
            {
                string URL = System.Web.Configuration.WebConfigurationManager.AppSettings["EMMA"].ToString();
                string EMMA = URL + "CheckLogin.aspx?seqno=" + sysid + "&page=2&login=";    //OE修改頁

                Sqlstr = @"SELECT Top 5 Agent_Name, Agent_Mail, UserID FROM DispatchSystem where Agent_Status != '離職' " +
                    "and Agent_Name in('陳志韋','簡子翔','沈志穎','溫孟君') ";
                if (OE_PS.IndexOf("台中") != -1 || OE_PS.IndexOf("臺中") != -1)
                    Sqlstr = @"SELECT Top 1 Agent_Name, Agent_Mail, UserID FROM DispatchSystem where Agent_Status != '離職' " +
                        "and Agent_Name in('陳志韋','簡子翔','沈志穎','呂子斌','溫孟君') ";
                var list = DBTool.Query<ClassTemplate>(Sqlstr);
                if (list.Count() > 0)
                {
                    sqlstr3 = @"INSERT INTO tblAssign ( AssignNo, E_MAIL, ConnURL ) VALUES ( @AssignNo, @E_MAIL, @ConnURL )";
                    foreach (var p in list)
                    {
                        ClassTemplate emma = new ClassTemplate()
                        {
                            //AssignNo = "【OE工程主管通知】【" + BUSINESSNAME + "】",   // + q.Agent_Name 
                            AssignNo = "【OE過審 通知工程主管派工】【" + BUSINESSNAME + "】",   // + q.Agent_Name 
                            ConnURL = EMMA + JASON.Encryption(p.UserID),
                            //ConnURL = EMMA + JASON.Encryption("admin"),   //測試用
                            E_MAIL = p.Agent_Mail,
                        };
                        using (IDbConnection db = DBTool.GetConn())
                            db.Execute(sqlstr3, emma);
                    }
                }
            }
            else if (Flag == "4")   //派工通知
            {
                string URL = System.Web.Configuration.WebConfigurationManager.AppSettings["EMMA"].ToString();
                string EMMA = URL + "CheckLogin.aspx?seqno=" + sysid + "&page=1&login=";    //OE審核頁

                Sqlstr = @"SELECT Top 2 Agent_Name, Agent_Mail, UserID FROM DispatchSystem where Agent_Status != '離職' " +
                    " and Agent_ID=@Agent_ID ";
                var list = DBTool.Query<ClassTemplate>(Sqlstr, new { Agent_ID = Enginner_ID });
                if (list.Count() > 0)
                {
                    sqlstr3 = @"INSERT INTO tblAssign ( AssignNo, E_MAIL, ConnURL ) VALUES ( @AssignNo, @E_MAIL, @ConnURL )";
                    foreach (var p in list)
                    {
                        ClassTemplate emma = new ClassTemplate()
                        {
                            AssignNo = "【OE派工通知】【" + BUSINESSNAME + "】",   // + q.Agent_Name 
                            ConnURL = EMMA + JASON.Encryption(p.UserID),
                            //ConnURL = EMMA + JASON.Encryption("admin"),   //測試用
                            E_MAIL = p.Agent_Mail,
                        };
            ...
            return JsonConvert.SerializeObject(new { status = "發生錯誤 錯誤原因"+(char)13+er });
        }
    }
    ...
    public static string Labor_Value(string SYSID, string Flag)             //      勞工的 無用
    {
        ...
        if (Flag == "2")
        {
            ...
            if (a.Any())
            {
                ...
                var language = Value(schedule.Labor_Country);
                switch (language)
                {
                    case "南非":
                        language = "英語";
                ...
                var b = a.ToList().Select(p => new
               {
                   ...
                   Labor_Country = Value(p.Labor_Country),
                   Labor_ID = p.Labor_ID,
    ...
    public static string Btn_Cancel_Click(string txt_cancel)                                    // 勞工服務 取消用
    {
        ...
        if (!String.IsNullOrEmpty(txt_cancel))
        {
            ...
            {
                txt_cancel = HttpUtility.HtmlEncode(Value(txt_cancel));
        ...
        string Hospital, string HospitalClass, string Question)
    {
        ...
        Contact_Co_TEL = Value(Contact_Co_TEL);
    ...
    public static string Load_OEData(string OE_Case_ID)                                                       // 新增 讀CaseData 案件資料
    {
        ...
        var lcd = DBTool.Query<T_0030010005_2>(sqlstr, new { OE_Case_ID = OE_Case_ID }).ToList().Select(p => new
        {
            ...
            ,Agent_Company = Value(Agent_Company)   
            ,Agent_Team = Value(Agent_Team)
            ,Chargeback = Value(p.Chargeback),
            Enginner = Value(p.Enginner),
            Enginner_ID = Value(p.Enginner_ID)   
        }); 
        string outputJson = JsonConvert.SerializeObject(lcd);
        return outputJson;
    }
    ...
    protected void btnUpload_Click(object sender, EventArgs e)  //可以上傳 
    {
        ...
        try
        {
            ...
            Thread.Sleep(500);  //延遲 好像沒作用
            System.Web.HttpContext.Current.Response.Redirect("~/0030010005.aspx?seqno=" + seqno);
        }
        catch (Exception ex)
        {
    ...
    bool savefile(FileUpload FileUpload1)
    {
        ...
        string A = @"C:\OE_File\" + seqno + (char)92;       //上傳的路徑加檔名.副檔名
    ...
    protected void Btn_Query_Click(object sender, EventArgs e)
    {
        ...
        Iframe1.Attributes["src"] = "/temp/" + report.GetView();
    }
    ...
    protected void Btn_exportExcel_Click(object sender, EventArgs e)
    {
        ...
        string OE_Case_ID = Request.Params["seqno"];
        var report = CreateReportRepository();
        if (report == null) return;
        Response.Clear();
        Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
        Response.AddHeader("Content-Disposition", string.Format("attachment; filename=【OE】商品訂單表 {0}.xlsx", OE_Case_ID));
    ...
    protected ReportRepository_010 CreateReportRepository()
    {
        try
        {
            ...
            string OE_Case_ID = Request.Params["seqno"];
            return new ReportRepository_010("", "", team, type, service, service_id, OE_Case_ID);
        }
        catch
        {
    ...
    //====================== 報表二
    protected void Btn_Query_Click2(object sender, EventArgs e)
    {
        var report2 = CreateReportRepository2();
        if (report2 == null)
        {
            Iframe2.Attributes["src"] = "";
            return;
        }
        Iframe2.Attributes["src"] = "/temp/" + report2.GetView();
    }
    protected void Btn_exportExcel_Click2(object sender, EventArgs e)
    {
        //DateTime date = DateTime.Parse(txt_E_DATETime.Text);
        DateTime date = DateTime.Now;
        string OE_Case_ID = Request.Params["seqno"];
        var report = CreateReportRepository2();
        if (report == null) return;
        Response.Clear();
        Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
        Response.AddHeader("Content-Disposition", string.Format("attachment; filename=【OE】商品訂單表 {0}.xlsx", OE_Case_ID));
    ...
    protected ReportRepository_011 CreateReportRepository2()
    {
        try
        {
            DateTime Sdate = DateTime.Now;
            ...
            if (Sdate > Edate)
            {
                RegisterStartupScript2("【開始時間】不能大於【結束時間】。");
                return null;
            }
            ...
            //string OE_Case_ID = "170703150318902";
            string OE_Case_ID = Request.Params["seqno"];
            return new ReportRepository_011("", "", team, type, service, service_id, OE_Case_ID);
        }
        catch
        {
            RegisterStartupScript2("【傳送系統參數錯誤】或【日期格式】不正確。");
            return null;
        }
    }

    private void RegisterStartupScript2(string msg)
    {
        if (ScriptManager.GetCurrent(this.Page) == null)
        {
            Page.ClientScript.RegisterStartupScript(this.Page.GetType(), "buttonStartup", "alert('" + msg + "');", true);
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Delete_OE(string sysid)
    {
        Check();
        string sqlstr = @"Delete OE_Data Where OE_D_ID=@SYSID";
    ...
    public static string List_Dispatch_Name()
    {
        string Sqlstr = @"SELECT DISTINCT Agent_ID, Agent_Name FROM DispatchSystem WHERE Agent_Status = '在職' " +
            " AND (Agent_Company in ('Engineer', 'Pre-sale') ) or Agent_Name='王筠婷' " +
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Load_Agent(string Agent_ID)
    {
        string Sqlstr = @"SELECT TOP 1 * FROM DispatchSystem WHERE Agent_ID = @Agent_ID AND Agent_Status != '離職'";      // 同員編且未離職
        var a = DBTool.Query<ClassTemplate>(Sqlstr, new { Agent_ID = Agent_ID }).ToList().Select(p => new
        {
            Agent_ID = p.Agent_ID,                      // 員工編號
            Agent_Name = p.Agent_Name,        // 人名
        });
        string outputJson = JsonConvert.SerializeObject(a);
        return outputJson;
    ...
    public class T_0030010005_2
    {
        ...
        public string Enginner_ID { get; set; }
