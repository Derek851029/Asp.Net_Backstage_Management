...
public partial class _0030010006 : System.Web.UI.Page
{
    ...
    public static string Case_Save(string sysid, string OE_PS, string Con_Name, string Con_Phone, //string SetupDate,
    ...
    // 新單子 數據登錄
    {
        ...
        string Sqlstr = @"INSERT INTO OE_Data (OE_Case_ID, OE_PS, Con_Name, Con_Phone,  " + //SetupDate,
    ...
    public static string Load_OEData(string OE_Case_ID)                                                       // 新增 讀CaseData 案件資料
    {
        ...
        var lcd = DBTool.Query<T_0030010015_2>(sqlstr, new { OE_Case_ID = OE_Case_ID }).ToList().Select(p => new
        {
            ...
            HardWare = p.HardWare
            ,
            SoftwareLoad = p.SoftwareLoad
            ,
            Name = p.Name
            ,
            PNumber = p.PNumber.Trim()
            ,
            S_APPNAME = p.S_APPNAME
            ,
            S_CONTACT_ADDR = p.S_CONTACT_ADDR
            ,
            S_APP_FTEL = p.S_APP_FTEL
            ,
            S_APP_MTEL = p.S_APP_MTEL
            ,
            S_HardWare = p.S_HardWare
            ,
            S_SoftwareLoad = p.S_SoftwareLoad
            ,
            RFQ = p.RFQ
            ,
            ...
            ,
            Warr_Time = p.Warr_Time
            ,
            Protect_Date = p.Protect_Date
            ,
            Prot_Time = p.Prot_Time
            ,
            Receipt_Date = p.Receipt_Date
            ,
            Receipt_PS = p.Receipt_PS
            ,
            Close_Out_Date = p.Close_Out_Date
            ,
