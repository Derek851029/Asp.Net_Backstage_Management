...
public partial class _0030010015 : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            if (seqno != "0")
            {
                ...
                if (!chk.Any())
                {
                    ...
                    Response.Write("<script>alert('查無【" + seqno + "】OE單編號'); location.href='/0030010000/0030010005.aspx'; </script>");
                }
                else
                {
...
/*                    if (schedule.Type_Value == "1")
    ...
    {
        //Check();
        string Sqlstr = @"SELECT PID,BUSINESSNAME,ID  FROM BusinessData group by PID,BUSINESSNAME,ID " +
            "  order by BUSINESSNAME, ID ";      //整理出可選擇的客戶列表
    ...
    {
        //Check();
        string sqlstr = @"SELECT DISTINCT Detail_Classified FROM OE_Product where Main_Classified = @Main ";
    ...
    public static string OE_List(string PID)                                                       // 新增 讀BusinessData 案件資料
    {
        ...
        {
            var lcd = DBTool.Query<T_0030010015_2>(sqlstr, new { PID = PID }).ToList().Select(p => new
            {
        ...
        //string outputJson = "";
        var lcd2 = DBTool.Query<T_0030010015_2>(sqlstr2, new { PID = PID, Agent_Company2 = Agent_Company1 }).ToList().Select(p => new
        {
            OE_Case_ID = Value(p.OE_Case_ID),
            BUSINESSNAME = Value(p.BUSINESSNAME),
            Name = Value(p.Name),
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string URL(string mno, string flag)
    {
        //Check();
        mno = mno.Trim();
        string str_url="";
        ...
        {
            return JsonConvert.SerializeObject(new { status = error + " seqno 非數字" });
        }

        if (mno.Length > 16 || mno.Length < 1)
        {
            return JsonConvert.SerializeObject(new { status = error + " seqno 不介於 1~16字元之間" });
        }

        if (mno != "0")
        {
            string sqlstr = @"SELECT TOP 1 OE_Case_ID FROM OE_Data WHERE OE_Case_ID=@Case_ID ";
            var a = DBTool.Query<ClassTemplate>(sqlstr, new { Case_ID = mno });

            if (!a.Any())
            {
                return JsonConvert.SerializeObject(new { status = "查無【" + mno + "】OE單編號。", type = "no" });
            };
        };
        if (flag=="1")
            str_url = "../0030010015.aspx?seqno=" + mno;         //打開 OE審核頁面
        else if (flag == "2")
            str_url = "../0030010005.aspx?seqno=" + mno;         //打開 OE修改頁面
    ...
    {
        string sqlstr2 = @" select * From [DimaxCallcenter].[dbo].[OE_Product] where Detail_Classified = @Detail ";
        var a = DBTool.Query<T_0030010015_2>(sqlstr2, new
        {
            Detail = Detail
        });
    ...
    public static string Delete(string OE_O_ID)
    {
        ...
        DBTool.Query<T_0030010015_2>(sqlstr, new { OE_O_ID = OE_O_ID });  //, UpdateTime = DateTime.Now
    ...
    public static string GetOEOrder(string OE_Case_ID)       //案件列表程式
    {
        ...
        string sqlstr2 = @" select * From OE_Order where OE_Case_ID = @OE_Case_ID and Delete_Date is null ";
        var a = DBTool.Query<T_0030010015_2>(sqlstr2, new
        {
            OE_Case_ID = OE_Case_ID
        });
        if (Agent_Company == "Engineer" || Agent_Company == "MA_Sales")
        {
            var b = a.ToList().Select(p => new
            {
                OE_O_ID = p.OE_O_ID,
    ...
    public static string OE_Total_Price(string OE_Case_ID)
    {
        ...
        var a = DBTool.Query<T_0030010015_2>(sqlstr, new { OE_Case_ID = OE_Case_ID }).ToList().Select(p => new
        {
    ...
    /*    [WebMethod(EnableSession = true)]//或[WebMethod(true)]           訂貨儲存
        public static int ALL(string ID)
        {
            ...
            var Team_Array = DBTool.Query<T_0030010015_2>(sqlstr, new { OE_Case_ID = ID });
        ...
        string Product_Name, string Main_Classified, string Detail_Classified, string T_Price)
    {
        ...
        var a = DBTool.Query<T_0030010015_2>(Sqlstr, new
        {
            OE_Case_ID = OE_Case_ID,
            OE_ID = ID,
            Quantity = Main,
    ...
    public static string Show_Client_Data(string value)                     // 叫出商業資料  234行
    {
        ...
        var a = DBTool.Query<T_0030010015_2>(Sqlstr, new { value = value }).ToList().Select(p => new
        {
            B = p.ID,
    ...
    public static string Show_Client_Data2(string value_2)                     // 叫出商業資料  234行
    {
        ...
        var a = DBTool.Query<T_0030010015_2>(Sqlstr, new { value_2 = value_2 }).ToList().Select(p => new
        {
            B = p.ID,
            C = p.Name,         // 公司名
            D = p.APPNAME,                  // 聯絡人
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Case_Save(string sysid, string OE_PS, string Con_Name, string Con_Phone, //新增OE 沒在用
        string UpdateDate, string Type_Value, string Type, string PID, string BUSINESSNAME,
        string APPNAME, string CONTACT_ADDR, string APP_FTEL, string APP_MTEL, string HardWare,
        string SoftwareLoad, string PNumber, string Name, string S_APPNAME, string S_CONTACT_ADDR,
    ...
    // 新單子 數據登錄
    {
        ...
        string Sqlstr = @"INSERT INTO OE_Data (OE_Case_ID, OE_PS, Con_Name, Con_Phone,  " + //UpdateDate,
           "  SetupDate, Type_Value, Type, PID, BUSINESSNAME, " +
           ...
           " Final_Price, Warranty_Date, Warr_Time, Protect_Date, Prot_Time,  " +
           " Receipt_Date, Receipt_PS, Close_Out_Date, Close_Out_PS ) " +

           " VALUES(@OE_Case_ID, @OE_PS, @Con_Name, @Con_Phone, " +    //@UpdateDate, 
           "  @SetupDate, @Type_Value, @Type, @PID, @BUSINESSNAME, " +
            " @APPNAME, @CONTACT_ADDR, @APP_FTEL, @APP_MTEL, @HardWare, " +
           "  @SoftwareLoad, @PNumber, @Name, @S_APPNAME, @S_CONTACT_ADDR, " +
           "  @S_APP_FTEL, @S_APP_MTEL, @S_HardWare, @S_SoftwareLoad, " +
    ...
    public static string Case_Save2(string sysid, string OE_PS, string Con_Name, string Con_Phone, //string SetupDate,
        string UpdateDate, string Type_Value, string Type, string PID, string BUSINESSNAME,
        string APPNAME, string CONTACT_ADDR, string APP_FTEL, string APP_MTEL, string HardWare,
        string SoftwareLoad, string PNumber, string Name, string S_APPNAME, string S_CONTACT_ADDR,
        string S_APP_FTEL, string S_APP_MTEL, string S_HardWare, string S_SoftwareLoad,
    ...
    //===========審查
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Review(string str_sysid, string Type)
    // 新單子 數據登錄
    {
        string Sqlstr = @"UPDATE OE_Data SET " +
            "Type = @Type " +
        ...
        string EMMA = URL + "CheckLogin.aspx?seqno=" + str_sysid + "&page=2&login=";
        //page 2  OE （修改） by CheckLogin
        string sqlstr = @"SELECT Top 1 Agent_Name, Agent_Mail, UserID FROM DispatchSystem where Agent_Status != '離職' "+
            " and Agent_Company ='Operation' and Agent_Team ='Operation' and Agent_Name='王筠婷' ";
        ...
        if (list.Count() > 0)
        {
            ...
            foreach (var q in list)
            {
                ...
                {
                    AssignNo = "【OE單通知】【序號:" + str_sysid + "OE單 已審核完畢】",   // + q.Agent_Name 
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Btn_Del_Click(string OE_Case_ID, string Back)
    {
        //Check();
        ...
        if (back_txt != "")
        {
            return JsonConvert.SerializeObject(new { status = back_txt });
        };

        string Agent_Name = HttpContext.Current.Session["UserIDNAME"].ToString();
        T_0030010015_2 template = new T_0030010015_2()
        {
            OE_Case_ID = OE_Case_ID,
            Type = "退單",
            Type_Value = "",  //  
        ...
        //=============  EMMA ============= 退單通知
        string URL = System.Web.Configuration.WebConfigurationManager.AppSettings["EMMA"].ToString();
        string EMMA = URL + "CheckLogin.aspx?seqno=" + OE_Case_ID + "&page=2&login=";

        sqlstr = @"Select BUSINESSNAME,Con_Company,Con_Name,UserID,Agent_Mail FROM OE_Data a left join DispatchSystem b "+
            " on a.Con_Name=b.Agent_Name and a.Con_Company=b.Agent_Company and Agent_Status='在職' "+
            " WHERE OE_Case_ID=@OE_Case_ID ";        // 抓 Agent_Mail UserID BUSINESSNAME 
        var list = DBTool.Query<ClassTemplate>(sqlstr, new { OE_Case_ID = OE_Case_ID });
        if (list.Count() > 0)
        {
            string sql = @"INSERT INTO [dbo].[tblAssign] " +
                "( [AssignNo], [E_MAIL], [ConnURL] ) " +
            ...
            foreach (var q in list)
            {
                ClassTemplate emma = new ClassTemplate()
                {
                    AssignNo = "【OE退單通知】【" + q.BUSINESSNAME + "】",   // "審核"  "派工"  "接單"  "暫結案"  "結案"
                    ConnURL = EMMA + JASON.Encryption(q.UserID),
                    E_MAIL = q.Agent_Mail
                };
                using (IDbConnection db = DBTool.GetConn())
    ...
    public static string Load_OEData(string OE_Case_ID)                                                       // 新增 讀CaseData 案件資料
    {
        ...
        var lcd = DBTool.Query<T_0030010015_2>(sqlstr, new { OE_Case_ID = OE_Case_ID }).ToList().Select(p => new
        {
