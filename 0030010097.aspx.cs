...
public partial class _0030010097 : System.Web.UI.Page
{
    ...
    protected string str_type = " 系統自動編號";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                Response.Redirect("/0030010000/0030010003.aspx?view=0");
            }
                ...
                else
                {
                    ...
                    if (schedule.Type_Value == "1")
                    {
                        ...
                        if (Session["Agent_LV"].ToString() != "30")
                        {
                            ...
                            {
                                //Response.Write("<script>alert('您沒有訪問此頁面的權限，此需求單非您部門的需求單。'); location.href='/0030010000/0030010003.aspx?view=0'; </script>");
                            }
                        }
                    }
/*                    else
                    {
                        Response.Redirect(url);       //這導向引發 bug !
                    }       //*/
                };
            }
            else
    ...
    {
        //Check();
        string Sqlstr = @"SELECT DISTINCT PID,BUSINESSNAME,ID FROM BusinessData order by PID ";
...
//            "HardWare, SoftwareLoad, SERVICEITEM, SalseAgent, CONTACT_ADDR, MEMO " +
    ...
    public static string Show_Client_Data2(string value_2)                     // 叫出商業資料  234行
    {
        ...
        var a = DBTool.Query<T_0030010097>(Sqlstr, new { value_2 = value_2 }).ToList().Select(p => new
        {
            B = p.ID,
            C = p.Name,         // 公司名
            D = p.APPNAME,                  // 聯絡人
    ...
    public static string List_Record(string id)
    {
        ...
        var a = DBTool.Query<T_0030010097>(Sqlstr, new { id = id }).ToList().Select(p => new
        {
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Reach(string sysid, string time, string reason, string Flag)
    {
        string Type_Value = "2";
        string Type = "處理中";
        string Time_01 = "";
        ...
        else if (Flag == "1")
        {
            ...
            if (String.IsNullOrEmpty(time))
                return JsonConvert.SerializeObject(new { status = "請填寫【補單時間】" });
            else if (String.IsNullOrEmpty(reason))
                return JsonConvert.SerializeObject(new { status = "請填寫【補單原因】" });
            string sql = @"UPDATE CaseData SET P_R_Time=getdate() WHERE Case_ID = @sysid";
            var b = DBTool.Query<ClassTemplate>(sql, new { sysid = sysid });    //儲存補登記的時間
        }

        string Sqlstr = @"UPDATE CaseData SET " +
            "Type = @Type, " +
            "Type_Value = @Type_Value, " +
            "Patch_Reach_Reason = @Flag_1 , " +
            "ReachTime = @ReachTime " +
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Finish(string sysid, string ps, string C_Name, string S_O_Type, string time, string reason, string Flag)
    {
        string Type_Value = "3";
        string Type = "已結案";
        string Time_02 = "";
        ...
        string UserIDNAME = HttpContext.Current.Session["UserIDNAME"].ToString();
        if (Flag == "0")
            Time_02 = DateTime.Now.ToString("yyyy/MM/dd HH:mm");
        else if (Flag == "1")
        {
            Time_02 = time;
            if (String.IsNullOrEmpty(time))
                ...
                return JsonConvert.SerializeObject(new { status = "請填寫【補單原因】" });
            string sql = @"UPDATE CaseData SET P_F_Time=getdate() WHERE Case_ID = @sysid";
            var b = DBTool.Query<ClassTemplate>(sql, new { sysid = sysid });    //儲存補登記的時間
        }
        ...
        string Sqlstr = @"UPDATE CaseData SET " +
            ...
            "Patch_Finish_Reason = @Flag_2, " +
            "PS = @PS " +   //, Service_Flag = '1'
            "WHERE Case_ID = @sysid";               //4+11+8+11         
        ...
        string mail = "";
        string Agent_Company = "%Engineer%";
        string Agent_Team = "%Manager%";

        string sqlstr = @"SELECT * FROM DispatchSystem WHERE Agent_Company like @Agent_Company and Agent_Team like @Agent_Team";
        var list = DBTool.Query<ClassTemplate>(sqlstr, new { 
            Agent_Company = Agent_Company, Agent_Team = Agent_Team });
        if (list.Any())
        {
        ...
        {
            AssignNo = "【結案通知】【" + C_Name + "：" + S_O_Type + "】",        // + service + "【填單人員：" + creat_name + "】
            E_MAIL = mail,
            ConnURL = EMMA
        };
...
/*    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    ...
    //=============【取消需求單】=============
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Btn_Cancel_Click(string txt_cancel)
    {
        //Check();
    ...
    public static string Load_Data(string Case_ID)
    {
        ...
        string outputJson = "";
        var chk = DBTool.Query<T_0030010097>(sqlstr, new { Case_ID = Case_ID });
        if (!chk.Any())
        {
 ...
 /*                   if (q.Create_Team != Agent_Team)
            ...
            outputJson = JsonConvert.SerializeObject(
                chk.ToList().Select(p => new
                {
                    ...
                    str_login_time = Value2(p.UploadTime), 
                    ...
                    str_i_ps = Value(p.Information_PS),
                    Agent_ID = Agent_ID
    ...
    {
        string Check = JASON.Check_ID("0030010099.aspx");       //檢查 ROLEPROG 的 Role_ID 以及 ROLEPROG 的 NAVIGATE_URL
        if (Check == "NO")
        {
    ...
    {
        //Check();
        string Sqlstr = @"SELECT DISTINCT Agent_ID, Agent_Name FROM DispatchSystem WHERE Agent_Status = '在職' AND Agent_Company=@Agent_Company ORDER BY Agent_ID, Agent_Name ";
    ...
    //============= 派工 =============
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Send(string sysid, string value, string PS, string C_Name, string S_O_Type)    //工單ID , 員工ID, 備註
    {
        //Check();
        string Type = "承辦中";
        string sqlstr = @"INSERT INTO Case_Agent ( UpdateTime, Case_ID, Agent_ID, Type, Agent_PS ) " +
        " VALUES( @UpdateTime, @Case_ID, @Agent_ID, @Type, @Agent_PS) ";
    ...
    //============= 轉派工 =============
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Change(string sysid, string value, string PS, string C_Name, string S_O_Type, string value2)    //工單ID , 員工ID, 備註
    {
        //Check();
        string Type = "承辦中";
        string sqlstr = @"INSERT INTO Case_Agent ( UpdateTime, Case_ID, Agent_ID, Type, Agent_PS ) " +
    ...
    {
        //Check();
        string Sqlstr = @"SELECT DISTINCT Agent_ID, Agent_Name FROM DispatchSystem WHERE Agent_Status = '在職' AND Agent_ID=@Agent_ID";
    ...
    //============= 簽核 =============
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Signed(string sysid, string value, string C_Name, string S_O_Type)    //工單ID , 員工ID, 備註
    {
        //Check();
        string Type = "已結案已簽核";
        string Type_Value = "4";
        string sqlstr = @"UPDATE CaseData set Type = @Type, Type_Value = @Type_Value " +
        ...
        string mail = "";
        sqlstr = @"SELECT Top 1 * FROM DispatchSystem WHERE Agent_ID=@value";
        var list = DBTool.Query<ClassTemplate>(sqlstr, new { value = value });
        if (list.Any())
        {
            ClassTemplate schedule = list.First();  // 功能???            
            mail = schedule.Agent_Mail;
        ...
        ClassTemplate emma = new ClassTemplate()
        {
            AssignNo = "【簽核完畢通知】【" + C_Name + "：" + S_O_Type + "】",        // + service + "【填單人員：" + creat_name + "】
            E_MAIL = mail,
            ConnURL = EMMA
        };
        sqlstr = @"INSERT INTO tblAssign (AssignNo ,E_MAIL ,ConnURL) VALUES(@AssignNo ,@E_MAIL ,@ConnURL)";
        using (IDbConnection db = DBTool.GetConn())
            db.Execute(sqlstr, emma);
        Thread.Sleep(250);
        return JsonConvert.SerializeObject(new { status = "signed" });
    }

    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string U_PS(string sysid, string ps)
    {
        string status = "修改失敗";
        string Sqlstr = @"UPDATE CaseData SET " +
            "PS = @ps " +
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string URL2(string mno)
    {
        //Check();
        mno = mno.Trim();
        string error = "傳送系統參數錯誤，請再嘗試或詢問管理人員，謝謝。";
    ...
    public static string Order(string Case_ID, string ID, string Main, string Price,
        string Product_Name, string Main_Classified, string Detail_Classified, string T_Price)
    {
        ...
        {
            return JsonConvert.SerializeObject(new { status = "【商品序號】錯誤" });
        }
        if (JASON.IsInt(Main) != true)
        {
            return JsonConvert.SerializeObject(new { status = error + "【 商品數量】 錯誤" });
        }
        if (JASON.IsInt(Price) != true)
        {
            return JsonConvert.SerializeObject(new { status = error + " 【商品價格】 錯誤" });
        }
        Sqlstr = @"INSERT INTO Case_Order (Case_ID, OE_ID, Unit_Price, Quantity, Product_Name, Main_Classified, Detail_Classified, T_Price ) " +
                " VALUES (@Case_ID, @OE_ID, @Unit_Price, @Quantity, @Product_Name, @Main_Classified, @Detail_Classified, @T_Price ) ";
        status = "new";
        var a = DBTool.Query<T_0030010097>(Sqlstr, new
        {
            Case_ID = Case_ID,
            OE_ID = ID,
            Quantity = Main,
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]       
    public static string GetCaseOrder(string Case_ID)       //案件列表程式
    {
        Case_ID = Case_ID.Trim();
        string sqlstr2 = @" select * From Case_Order where Case_ID = @Case_ID and Delete_Date is null";
        var a = DBTool.Query<T_0030010097>(sqlstr2, new
        {
            Case_ID = Case_ID
        });
        var b = a.ToList().Select(p => new
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Delete(string Case_O_ID)
    {
        //Check();
        Case_O_ID = Case_O_ID.Trim();
        //string sqlstr = @"delete FROM OE_Order where OE_O_ID = @OE_O_ID ";
        string sqlstr = @"Update Case_Order set Delete_Date = getdate() WHERE Case_O_ID = @Case_O_ID";
        DBTool.Query<T_0030010097>(sqlstr, new { Case_O_ID = Case_O_ID });  //, UpdateTime = DateTime.Now
        return JsonConvert.SerializeObject(new { status = "success" });
    }

    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string image_Click()
    {
        string CNo = HttpContext.Current.Session["CNo"].ToString();
        return "../Report/Image_Load.aspx?seqno=" + CNo;
    }
    ...
    protected void btnUpload_Click(object sender, EventArgs e)  //可以上傳 
    {
        ...
        try
        {
            ...
            System.Web.HttpContext.Current.Response.Redirect("~/0030010097.aspx?seqno=" + seqno);
        }
        catch (Exception ex)
        {
    ...
    public class T_0030010097
    {
        public int a { get; set; }
        public string ID { get; set; }
        public string OE_ID { get; set; }
        public string Case_O_ID { get; set; }
