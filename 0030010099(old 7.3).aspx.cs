...
public partial class _0030010099 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                Session["MNo"] = seqno;
                sql_txt = @"SELECT TOP 1 SYS_ID FROM LaborTemplate WHERE MNo=@MNo";
                ...
                {
                    ClassTemplate schedule = ClassScheduleRepository._0030010001_View_MNo(seqno);
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Case_Save(string sysid, string login_time, string client_id, string urgency, string op_type, string esti_fin_time, string onspot_time, string opinion, string rep_type, string ps, string deal_process, string rep, int telecomm_id)
    {
        DateTime time = DateTime.Parse(login_time);
        string Sqlstr = @"INSERT INTO CaseData (Case_ID,Upload_Time, ID, Urgency, EstimatedFinishTime, OpinionType, OnSpotTime, OpinionContent, ReplyType, PS, DealingProcess, Reply, Telecomm_ID ) " +
           " VALUES( @sysid, @login_time, @client_id, @urgency, @esti_fin_time, @op_type, @onspot_time,@opinion,@rep_type,@ps,@deal_process,@rep,@telecomm_id )";
        string status = "登單完成";
    ...
    //=============【帶入需求單 母單內容】=============
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Load_MNo(string MNo)
    {
        Check();
