...
//            "HardWare, SoftwareLoad, SERVICEITEM, SalseAgent, CONTACT_ADDR, MEMO " +
    ...
    public static string Case_Save(string sysid, string sysid2 ,string login_time, string Caller_ID, string Creat_Agent,       //4
        ...
        string Telecomm_ID, string APP_EMAIL, string HardWare, string SoftwareLoad, string SERVICEITEM, string Information_PS,     //4+11
        ...
        string rep )    //  string type,    string ps,
    {
        ...
        if (a.Any())
        {
            //return JsonConvert.SerializeObject(new { status = "已有相同案件編號，請利用產生補單編號鈕產生新的案件編號。" });  //偶爾會故障重複 導致無法新增 所以移除
        };
        ...
        {
            return JsonConvert.SerializeObject(new { status = "請選擇【緊急程度】" });
        }
        if (String.IsNullOrEmpty(op_type))
        {
            return JsonConvert.SerializeObject(new { status = "請選擇【意見類型】" });
        }
        if (!String.IsNullOrEmpty(opinion))
        {
            if (opinion.Length > 1000)
            {
                return JsonConvert.SerializeObject(new { status = "【意見內容】不能超過１０００個字元。" });
            }
        }
        else
            return JsonConvert.SerializeObject(new { status = "請填寫【意見內容】" });
        ...
        Sqlstr = @"INSERT INTO CaseData ( Case_ID, C_ID2, SetupTime, Caller_ID, Creat_Agent, " +      //4
            ...
            "OpinionContent, Type_Value, Type, Handle_Agent, Reply, UploadTime, Agent_ID, UserID, Information_PS ) " +                   //4+11+8+11  
            // PS, 
            ...
            "@opinion, @Type_Value, @type, @Handle_Agent, @rep, @UpdateTime, @A_ID, @UserID, @Information_PS ) ";                  //4+11+8+11         
...
/*    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
 ...
 /*                   if (q.Create_Team != Agent_Team)
    ...
    public static string Send_Save(string sysid, string value, string C_Name, string S_O_Type)    //工單ID , 員工ID, 備註
    {
        ...
        string mail = "";
        string Agent_Name = "";
        sqlstr = @"SELECT Top 1 * FROM DispatchSystem WHERE Agent_ID=@Agent_ID and Agent_Status='在職' ";
...
/*        string Sqlstr = @"UPDATE CaseData SET Handle_Agent = @Agent_Name, " + // 新增頁面前無資料
    ...
    public static string Change(string sysid, string value, string C_Name, string S_O_Type, string value2)    //工單ID , 員工ID, 備註
    {
        ...
        string mail2 = "";
        string name = "";
        sqlstr = @"SELECT Top 1 * FROM DispatchSystem WHERE Agent_ID=@Agent_ID and Agent_Status='在職' ";  // 抓派工員工信箱
        var list = DBTool.Query<ClassTemplate>(sqlstr, new { Agent_ID = value });
        if (list.Any())
        {
            ClassTemplate schedule = list.First();  // 功能???            
        ...
        sqlstr = @"SELECT Top 1 * FROM DispatchSystem WHERE Agent_ID=@Agent_ID and Agent_Status='在職' ";  // 抓取消員工信箱
    ...
    bool savefile(FileUpload FileUpload1)
    {
        ...
        string A = @"D:\網站上傳檔案\File\Dimax\" + seqno + (char)92;       //上傳的路徑加檔名.副檔名
