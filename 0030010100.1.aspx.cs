...
public partial class _0030010100 : System.Web.UI.Page
{
 ...
 /*               else
    ...
    public static string Show_Client_Data(string value)                     // 叫出商業資料  234行
    {
        ...
        string Sqlstr = @"SELECT BUSINESSNAME,APPNAME,APP_OTEL_AREA,APP_OTEL,APP_MTEL,HardWare, " +
            " SoftwareLoad,SERVICEITEM,Telecomm_ID FROM BusinessData WHERE ID=@value ";                                                                //  ID=@value ?
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Eng_Name_Search()
    {
        Check();
        string Sqlstr = @"SELECT DISTINCT Agent_Name, Agent_ID FROM AgentBase ";
        var a = DBTool.Query<ClassTemplate>(Sqlstr).ToList().Select(p => new
        {
            A = p.Agent_Name,
            B = p.Agent_ID
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Eng_Name_Search_Reg()
    {
        Check();
        string Sqlstr = @"SELECT DISTINCT Agent_Name, Agent_ID FROM AgentBase ";
        var a = DBTool.Query<ClassTemplate>(Sqlstr).ToList().Select(p => new
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Case_Save_Maintenance(string sysid, string maintenance_eng_id, int maintenance_case_status)
   // 新單子 數據登錄
    {
        string Sqlstr = @"INSERT INTO Maintenance_Case (Case_ID,Engineer_in_Charge,Case_Status) VALUES( @sysid, @maintenance_eng_id, @maintenance_case_status )";
                       
        string status = "新增維護單完成";
        var a = DBTool.Query<ClassTemplate>(Sqlstr, new
        {
            sysid = sysid,
            maintenance_eng_id = maintenance_eng_id,
            maintenance_case_status = maintenance_case_status,
