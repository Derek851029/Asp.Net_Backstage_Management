...
public partial class _0030010105 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                Session["MNo"] = seqno;
                sql_txt = @"SELECT TOP 1 SYS_ID FROM CaseData WHERE Case_ID=@Case_ID";
                var chk = DBTool.Query<ClassTemplate>(sql_txt, new { MNo = seqno });
                if (!chk.Any())
                {
                    //Response.Redirect("/0030010000/0030010002.aspx");
                    Response.Write("<script>alert('查無【" + seqno + "】母單編號'); location.href=/Default.aspx'; </script>");
                }
                else
                {
...
//            "HardWare, SoftwareLoad, SERVICEITEM, SalseAgent, CONTACT_ADDR, MEMO " +
    ...
    public static string Show_Client_Data2(string value_2)                     // 叫出商業資料  234行
    {
        ...
        var a = DBTool.Query<T_0030010105>(Sqlstr, new { value_2 = value_2 }).ToList().Select(p => new
        {
            B = p.ID,
            C = p.Name,         // 公司名
            D = p.APPNAME,                  // 聯絡人
    ...
    {
        Check();
        string Sqlstr = @"SELECT Case_ID, Upload_Time, BUSINESSNAME, OpinionContent, ReplyType, Reply, DealingProcess " +
            "FROM CaseData as a LEFT JOIN BusinessData as b on a.ID=b.PID WHERE PID=@id ";
        var a = DBTool.Query<ClassTemplate>(Sqlstr, new { id = id }).ToList().Select(p => new
        {
            A = p.Case_ID,
            B = Convert.ToDateTime(p.Upload_Time).ToString("yyyy/MM/dd HH:mm"),
            C = p.BUSINESSNAME,
            D = p.OpinionContent,
            E = p.ReplyType,
            F = p.Reply,
            G = DealingProcess(p.DealingProcess.Trim())
        });
        string outputJson = JsonConvert.SerializeObject(a);
        return outputJson;
    }
    ...
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Case_Save(string sysid, string login_time, string Caller_ID, string Creat_Agent,       //4
        ...
        string opinion, string type, string Handle_Agent, string ps,                    //4+11+8+8
        string rep )
    {
        ...
        string Sqlstr = @"INSERT INTO CaseData ( Case_ID, SetupTime, Caller_ID, Creat_Agent, " +      //4
            ...
            "Urgency, OnSpotTime, OpinionType, ReplyType, " +
            "OpinionContent, Type, Type_Value, Handle_Agent, PS, Reply, UploadTime ) " +                   //4+11+8+11  

            " Values (@sysid, @Setuptime, @Caller_ID, @Creat_Agent, " +       //4
            "@PID, @ID, @BUSINESSNAME, @ADDR, @APPNAME, @APP_MTEL, " +
            "@Telecomm_ID, @APP_EMAIL, @HardWare, @SoftwareLoad, @SERVICEITEM, " +     //4+11
            "@PNumber, @Name, @APPNAME_2, @ID_2, " +
            "@APP_MTEL_2, @ADDR_2, @HardWare_2, @SoftwareLoad_2, " +            //4+11+8
            "@urgency, @onspot_time, @op_type, @rep_type, " +
            "@opinion, @type, @Type_Value, @Handle_Agent, @ps, @rep, @UpdateTime ) ";                  //4+11+8+11  
       
        string status = "new";

        var a = DBTool.Query<ClassTemplate>(Sqlstr, new
        {
            sysid = sysid, Setuptime = Setuptime, Caller_ID = Caller_ID, Creat_Agent = Creat_Agent,       //4
            ...
            opinion = opinion, type = type, Handle_Agent = Handle_Agent, ps = ps, rep = rep,                   //4+11+8+8
    ...
    //=============【帶入需求單 母單內容】=============
    [WebMethod(EnableSession = true)]//或[WebMethod(true)]
    public static string Load_Data(string MNo)
    {
        Check();
    ...
    {
        string Check = JASON.Check_ID("0030010105.aspx");
        if (Check == "NO")
        {
    ...
    public static string Send(string sysid, string value, string PS, string C_Name, string S_O_Type)    //工單ID , 員工ID, 備註
    {
        ...
        //string EMMA = URL + "CheckLogin.aspx?seqno=" + sysid + "&page=" + page + "&login=" + JASON.Encryption(login);
        ClassTemplate emma = new ClassTemplate()
       {
