...
public partial class _0060010005 : System.Web.UI.Page
{
    ...
    public static string SearchCase(string start, string end, string A_N, string D_P)
    {
        //Check();
        string outputJson = "";
        if (!string.IsNullOrEmpty(end))
        {
            if (A_N != "" && D_P != "")
            {
                ...
                var a = DBTool.Query<T_0060010005>(sqlstr, new { start = start, end = end, Agent_ID = A_N, Type = D_P }).ToList().Select(p => new
                {
                    SetupTime = Value2(p.SetupTime),
                    Case_ID = Value(p.Case_ID),
                    BUSINESSNAME = Value(p.BUSINESSNAME),
            ...
            else if (A_N != "" && D_P == "")
            {
                ...
                var a = DBTool.Query<T_0060010005>(sqlstr, new { start = start, end = end, Agent_ID = A_N }).ToList().Select(p => new
                {
                    SetupTime = Value2(p.SetupTime),
                    Case_ID = Value(p.Case_ID),
                    BUSINESSNAME = Value(p.BUSINESSNAME),
            ...
            else if (A_N == "" && D_P != "")
            {
                ...
                var a = DBTool.Query<T_0060010005>(sqlstr, new { start = start, end = end, Type = D_P }).ToList().Select(p => new
                {
                    SetupTime = Value2(p.SetupTime),
                    Case_ID = Value(p.Case_ID),
                    BUSINESSNAME = Value(p.BUSINESSNAME),
            ...
            else
            {
                ...
                var a = DBTool.Query<T_0060010005>(sqlstr, new { start = start, end = end }).ToList().Select(p => new
                {
                    SetupTime = Value2(p.SetupTime),
                    Case_ID = Value(p.Case_ID),
                    BUSINESSNAME = Value(p.BUSINESSNAME),
