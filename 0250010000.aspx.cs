        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                string Sqlstr = @"select count(*) as Flag FROM [InSpecation_Dimax].[dbo].[Mission_Case] where " +
                    " Year=year(@time) and Month=month(@time)";
...
//            "HardWare, SoftwareLoad, SERVICEITEM, SalseAgent, CONTACT_ADDR, MEMO " +
        ...
        string id, string t_id, string addr, string name, string mtel, string cycletime, string agent )
    {
        ...
        Sqlstr = @"SELECT Top 1 mission_name From InSpecation_Dimax.dbo.Mission_Title  " +
                        " Where SYSID=@SYSID ";
        var list = DBTool.Query<ClassTemplate>(Sqlstr, new { SYSID = id });
        if (list.Any())
        {
            ClassTemplate schedule = list.First();
            Title_Name = schedule.mission_name;
        }

        Sqlstr = @"INSERT INTO [InSpecation_Dimax].[dbo].[Mission_Case] ( Case_ID, SetupTime, Creat_Agent, Title_Name, Title_ID, " +      //4
            " Telecomm_ID, ADDR, APPNAME, APP_MTEL, " +     //8
            " Cycle, Agent_ID, Type_Value, Type,  Year, Month) " +                   //12
            // PS, 
            " Values (@sysid2, @Setuptime, @Creat_Agent, @Title_Name, @Title_ID, " +       //4
            " @Telecomm_ID, @ADDR, @APPNAME, @APP_MTEL, " +
            " @Cycle, @Agent_ID, @Type_Value, @Type, year(@Time_01), month(@Time_01) ) ";                  //12
        ...
        if (!string.IsNullOrEmpty(agent))   // 當值為null時跳過  非 null 時 update
        {
            Sqlstr = @"UPDATE [InSpecation_Dimax].[dbo].[Mission_Case] SET " +
                ...
                "Mission_Case.UserID = DispatchSystem.UserID " +
        ...
        string id, string t_id, string addr, string name, string mtel, string cycletime, string agent) //
    {
        ...
        string UserIDNAME = HttpContext.Current.Session["UserIDNAME"].ToString();
        string Title_Name = "";
        string sqlstr = @"SELECT Top 1 mission_name From InSpecation_Dimax.dbo.Mission_Title  " +
                        " Where SYSID=@SYSID ";
        var list = DBTool.Query<ClassTemplate>(sqlstr, new { SYSID = id });
        if (list.Any())
        {
        ...
        {
            Sqlstr = @"UPDATE [InSpecation_Dimax].[dbo].[Mission_Case] SET Title_Name = @Title_Name, Title_ID = @Title_ID WHERE Case_ID = @sysid2";
            a = DBTool.Query<ClassTemplate>(Sqlstr, new
            {
                sysid2 = sysid2,
                Title_Name = Title_Name,
        ...
        if (!string.IsNullOrEmpty(agent))   // 當值為null時跳過  非 null 時 update
        {
            Sqlstr = @"UPDATE [InSpecation_Dimax].[dbo].[Mission_Case] SET " +
                ...
                "Mission_Case.Handle_Agent = DispatchSystem.Agent_Name " +
                "FROM [DimaxCallcenter].[dbo].[DispatchSystem]  WHERE Mission_Case.Case_ID = @sysid2 and DispatchSystem.Agent_ID = @Agent_ID "+
                "And DispatchSystem.Agent_Status != '離職' ";
            a = DBTool.Query<ClassTemplate>(Sqlstr, new
            {
                sysid2 = sysid2,
                Agent_ID = agent,
