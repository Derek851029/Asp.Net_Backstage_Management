...
/// 派工系統 - 以雇主服務統計 ( 前 10 家 )
/// </summary>
public class ReportRepository_011
{
    #region 屬性
    ...
    private string OE_Case_ID { get; set; }
    private object whereobject
    {
        get
        {
            if (string.IsNullOrEmpty(Create_Team))
                return new { StartDate, EndDate, Type_Value, Service, Service_ID, OE_Case_ID };
            else
                return new { StartDate, EndDate, Create_Team, Type_Value, Service, Service_ID, OE_Case_ID };
        }
    }
    ...
    private string QuerySqlStr
    {
        get
        {
            ...
            string Sqlstr = @"select * from OE_Order where Delete_Date is null {0} ";   //{0} 第一行
            return string.Format(Sqlstr,
                string.IsNullOrEmpty(OE_Case_ID) ? string.Empty : " AND OE_Case_ID=@OE_Case_ID "
    ...
    public ReportRepository_011(string StartDate, string EndDate, string Create_Team, string Type_Value, string Service, string Service_ID, string OE_Case_ID)
    {
    ...
    {
        ISheet sheet = workbook.CreateSheet("【派工系統】需求單明細表");
        int colindex = 0;
        int rowindex = 0;
        ...
        foreach (var item in datagroup)
        {
            ...
            row.CreateCell(colindex++).SetCellValue("商品編號");
            row.CreateCell(colindex++).SetCellValue("商品名稱");
            row.CreateCell(colindex++).SetCellValue("商品數量");
            //row.CreateCell(colindex++).SetCellValue("商品單價");
            //row.CreateCell(colindex++).SetCellValue("總價");
            ...
            row.CreateCell(0).SetCellValue("【OE】商品訂單表 " + OE_Case_ID + "   共 " + total + " 筆");
