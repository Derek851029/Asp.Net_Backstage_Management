...
<asp:Content ID="Content" ContentPlaceHolderID="head2" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function List_Schedule(){
            $.ajax({
                url: 'Default.aspx/List_Schedule',
                type: 'POST',
                        ...
                        columns: [
                            ...
                            { data: "Visit_Date" },
                            {
                                data: "Case_SYSID", render: function (data, type, row, meta) {
                                    return "<button type='button' id='search_"+data+"' class='btn btn-info btn-lg'>前往案件</button>";
                                }
                            }
                        ]
                    });
                    $('button[id^="search_"]').click(function () {
                        let Case_SYSID = $(this).attr('id').split('_')[1]
                        console.log(Case_SYSID)
                        window.location.href = './2021_Case/0010010005.aspx?seqno=' + Case_SYSID + ''
                    })
                }
            });
        ...
        function Check_Prot_Time() {    //檢查維護終止日並寄信
            $.ajax({
                url: 'Default.aspx/Check_Prot_Time',
                type: 'POST',
                //data: JSON.stringify({ mno: OE_Case_ID }),
                contentType: 'application/json; charset=UTF-8',
                dataType: "json",
        ...
        function Alert_Table() {            //個人逾期案件列表
            $.ajax({
                url: 'Default.aspx/Alert_Table_List',
                type: 'POST',
                //async: false,
                ...
                success: function (doc) {
                    ...
                    $('#data_A tbody').unbind('click').
                        on('click', '#edit', function () {
                            var Case_ID = table.row($(this).parents('tr')).data().Case_ID.toString();
                            //URL(mno);
                            window.location = "../0030010097.aspx?seqno=" + Case_ID;
                        });
                }
            });
        }
                    ...
                    success: function (doc) {
                            ...
                            columns: [
                                    ...
                                    { data: "E" },
                                    {
                                        data: "D", render: function (data, type, row, meta) {
                                            if (row.D != "休假申請" && row.D != "休假申請(上午)" && row.D != "休假申請(下午)") {
                                                return "";
                                            } else {
                        ...
                        $('#tab_Record tbody').unbind('click').
                            on('click', '#button', function () {
                                ...
                                var type = table.row($(this).parents('tr')).data().D;
                                Accept(cno, type);
                            })
            ...
            function Accept(DI_No, Type) {
                if (Type == "休假申請(上午)") Type = "1";
                else if (Type == "休假申請(下午)") Type = "2";
                else Type = "0";

                //alert("A=" + DI_No + "   T=" + Type);
                if (confirm("確定要接受嗎？")) {
                    $.ajax({
                        ...
                        type: 'POST',
                        //async: false,
                        data: JSON.stringify({ DI_No: DI_No, Type: Type }),
                        contentType: 'application/json; charset=UTF-8',
                        dataType: "json",       //如果要回傳值，請設成 json
    ...
    <div class="table-responsive" style="width: 95%; margin: 10px 20px" runat="server">
        ...
        <%--<button type="button" id="List_Schedule" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#dialog1">
            <span class='glyphicon glyphicon-plus'></span>&nbsp;&nbsp;顯示行事曆
        </button>--%>
            <div style="float: right;">
                ...
                <%--  =========== 逾期提示清單 ===========--%>
    <div id="div_Alert" class="table-responsive" style="width: 1280px; margin: 10px 20px" runat="server">
        <h2><strong>服務單列表</strong></h2>
       <table  id="data_A" class="display table table-striped" style="width: 99%">
            <thead>
                <tr>
                    <th style="text-align: center;" width: 10%;><strong>登錄日期</strong></th>
    ...
    <div id="table_data" class="table-responsive" style="width: 95%; margin: 10px 20px" runat="server">
        ...
        <table id="tab_Record" class="display table table-striped" style="width: 99%">
            <thead>
                <tr>
                    <th style="text-align: center;">休假開始日</th>
                    <th style="text-align: center;">休假結束日</th>
    ...
    <div id="table_data2" class="table-responsive" style="width: 95%; margin: 10px 20px" runat="server">
    ...
    <%--===========================行事曆dialog=======================================--%>
<div class="modal" id="dialog1" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" style="width: auto;height:auto">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h2 class="modal-title"><strong>以下為您今日行程</strong></h2>
            </div>
                ...
                <table id="data4" class="display table table-striped" style="width: 99%">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: center;">拜訪事項</th>
                            <th style="text-align: center;">拜訪時程</th>
