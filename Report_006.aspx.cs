...
public partial class Report_006 : System.Web.UI.Page
{
    ...
    {
        string sqlstr = "SELECT Agent_Team FROM [Faremma].[dbo].CASEDetail WHERE Agent_Team != '' "
            + " GROUP BY Agent_Team "
            + " ORDER BY charindex ( Agent_Team, ( SELECT TOP 1 Team FROM [Faremma].[dbo].Team_Rank ) ) ";

        var list = DBTool.Query<ClassTemplate>(sqlstr).ToList().Select(p => new { value = p.Agent_Team, text = p.Agent_Team });
    ...
    protected void Btn_exportExcel_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("Content-Disposition", string.Format("attachment; filename={0}月【派工系統】雇主服務統計報表.xlsx", date.Month));
        Response.BinaryWrite(report.GetReport());
        Response.Flush();
        Response.End();
    }

    protected ReportRepository_006 CreateReportRepository()
    {
        try
        {
            ...
            report_title.InnerText = DateTime.Parse(txt_S_DATETime.Text).ToString("yyyy年MM月dd日") + " 至 " + DateTime.Parse(txt_E_DATETime.Text).ToString("yyyy年MM月dd日") + "【" + team + "】" + "雇主服務統計報表";
            return new ReportRepository_006(txt_S_DATETime.Text, txt_E_DATETime.Text, team);
        }
        catch
        {
