...
public partial class Report_012 : System.Web.UI.Page
{
    ...
    protected void List_Team()
    {
        ...
        var list = DBTool.Query<ClassTemplate>(sqlstr).ToList().Select(p => new { value = p.PID, text = "【" + p.PID + "】" + p.BUSINESSNAME });
    ...
    {
        string sqlstr = "Select LTRIM(RTRIM(Handle_Agent))as Handle_Agent "+
            "FROM CaseData Where Handle_Agent<>'' group by LTRIM(RTRIM(Handle_Agent)) ";
   ...
   protected void drop_service_change(object sender, EventArgs e)
    ...
    protected ReportRepository_012 CreateReportRepository()
    {
        try
        {
            ...
            string o_content = "";//停用
            return new ReportRepository_012(txt_S_DATETime.Text, txt_E_DATETime.Text, business, type, enginner, o_type, o_content);
        }
        catch
        {
