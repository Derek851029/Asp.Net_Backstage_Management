...
public partial class Report_013 : System.Web.UI.Page
{
    ...
    protected void List_Team()
    {
        ...
        var list = DBTool.Query<ClassTemplate>(sqlstr).ToList().Select(p => new { value = p.PID, text = "【"+p.PID+"】"+p.BUSINESSNAME });//text = "【"+p.PID+"】"+p.BUSINESSNAME
        ...
        drop_businessname.Items.Insert(0, new ListItem("全客戶", ""));
    }

    protected void List_Service()
    {
        string sqlstr = "Select a.Agent_ID, a.UserID, b.Agent_Name From CaseData as a Left Join DispatchSystem as b "+
            " on a.Agent_ID=b.Agent_ID and a.UserID=b.UserID Where a.Agent_ID<>''and a.UserID<>''and b.Agent_Name<>'' "+
            " group by a.Agent_ID, a.UserID, b.Agent_Name ";
        var list = DBTool.Query<ClassTemplate>(sqlstr).ToList().Select(p => new { value = p.Agent_Name, text = p.Agent_Name });
    ...
    protected void List_OpinionType()
    {
   ...
   protected void drop_service_change(object sender, EventArgs e)
    ...
    protected void Btn_exportExcel_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("Content-Disposition", string.Format("attachment; filename={0}月【服務單報表】.xlsx", date.Month));
        Response.BinaryWrite(report.GetReport());
        Response.Flush();
        Response.End();
    }

    protected ReportRepository_013 CreateReportRepository()
    {
        try
        {
            ...
            string txt_b_name = "%" + txt_businessname.Text + "%";
            string type = drop_type.SelectedValue;
            string enginner = drop_enginner.SelectedValue;
            string o_type = drop_opinion_type.SelectedValue;
            string o_content = "%" + txt_opinioncontent.Text +"%";
            return new ReportRepository_013(txt_S_DATETime.Text, txt_E_DATETime.Text, business, type, enginner, o_type, o_content, txt_b_name);
        }
        catch
        {
