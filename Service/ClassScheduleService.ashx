...
public class ClassScheduleService : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (date.Day >= 25)
        {
            ...
            context.Response.Write(JsonConvert.SerializeObject(new { status = "success", msg = "日期超過25號，產生" + month + "月維護單" }));
        ...
        else
        {
            ...
            context.Response.Write(JsonConvert.SerializeObject(new { status = "success", msg = "日期未於25號之後，產生" + date.Month + "月維護單" }));
        }

        /*string checkstr = @"select top 1 SYS_ID from [Faremma].[dbo].[Rep_Classes2016] where WORK_DATE between @START_TIME AND @END_TIME";
        ...
        try
        {
            ...
            context.Response.Write(JsonConvert.SerializeObject(new
            {
                status = "success",
                msg = string.Empty
                    //+ "，新增 維護單 資料完成，起始時間：" + START_TIME + "，終止時間：" + END_TIME
                    + "，新增 維護單 資料完成" 
            }));
        }
        catch (Exception err)
