...
public class Daily_Email : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string str_AssignNo2 = "";    //1000字 要上調配合多筆同發
        string str_AssignNo3 = "";    //1000字 要上調配合多筆同發
        string str_URLS = "";
        int i;
        int Count = 0;
...
order by C_ID2 
            ...
            if (list.Count() > 0)   //找逾期案件
            {
                ...
                Sqlstr = @"Select Agent_Mail From DispatchSystem Where Agent_Status='在職'and " +
                        " Agent_Name in('簡子翔','溫孟君') "; //2020 05 15
            ...
            //  && (date.Month == 1 || date.Month == 4 || date.Month == 7 || date.Month == 10)
            {
                //維護時間逾期通知開始
                EMMA_URL = EMMA + "0060010001.aspx";

                sqlstr = @"Select CONVERT(varchar,PID)+' '+BUSINESSNAME+' 將於'+CONVERT(varchar,Month(Prot_Time))+'月 維護到期'as BUSINESSNAME, 
Prot_Time, PID FROM BusinessData WHERE Type='保留' AND Prot_Time<getdate()+92 
AND Prot_Mail = '0' Order by Prot_Time ";
                ...
                if (list3.Count() > 0)
                {
                    ...
                    {
                        str_AssignNo2 = str_AssignNo2 + "【客戶:" + s.BUSINESSNAME + "】" + (char)10;
                    }
                    logger.Info("維護即將逾期名單:" + str_AssignNo2);
                    ...
                    if (list4.Any())
                    {
                        foreach (var t in list4)
                        {
                            ...
                            {
                                logger.Info("Email = " + t.Agent_Mail );
                                Sql = @"INSERT INTO tblAssign (AssignNo, E_MAIL, ConnURL) VALUES(@AssignNo ,@E_MAIL ,@ConnURL) ";
                                ClassTemplate emma = new ClassTemplate()
                                {
                                    AssignNo = "【維護到期通知】" + (char)10 + str_AssignNo2,
                                    ConnURL = EMMA_URL,
                                    E_MAIL = t.Agent_Mail,
                                };
                                using (IDbConnection db = DBTool.GetConn())
                    ...
                    foreach (var s in list3)
                    {
                        logger.Info("改 Prot_Mail" + s.PID);
                        Sql = @"UPDATE BusinessData SET Prot_Mail = '1' where PID=@PID ";
                        using (IDbConnection db = DBTool.GetConn())
                            db.Execute(Sql, new { PID = s.PID });    //*/
                    }
                    context.Response.Write(JsonConvert.SerializeObject(new
                    {
                        status = "success",
                ...
                //保固時間逾期通知開始
                sqlstr = @"Select CONVERT(varchar,PID)+' '+BUSINESSNAME+' 將於'+CONVERT(varchar,Month(Warr_Time))+'月 保固到期'as BUSINESSNAME,
Warr_Time, PID FROM BusinessData WHERE Type='保留' AND Warr_Time<getdate()+92
AND (Prot_Time is null or Prot_Time<Warr_Time) AND Warr_Mail = '0' order by Warr_Time ";                
                var list5 = DBTool.Query<ClassTemplate>(sqlstr);
                if (list5.Count() > 0)
                {
                    foreach (var u in list5)
                    {
                        str_AssignNo3 = str_AssignNo3 + "【客戶:" + u.BUSINESSNAME + "】" + (char)10;
                    }
                    Sqlstr = @"SELECT Top 3 * FROM DispatchSystem WHERE Agent_Status='在職' And  " +
                            //" Agent_ID in('0002','0003','0004') "; // 192.168.2.170 用
                            //" Agent_Name in('測試用員工','系統管理員','德瑪測試') "; // 德 測試
                    ...
                    if (list6.Any())
                    {
                        foreach (var v in list6)
                        {
                            ...
                            {
                                logger.Info("Email = " + (char)10+ v.Agent_Mail );
                                Sql = @"INSERT INTO tblAssign (AssignNo, E_MAIL, ConnURL) VALUES(@AssignNo ,@E_MAIL ,@ConnURL) ";
                                ClassTemplate emma = new ClassTemplate()
                                {
                                    AssignNo = "【保固到期通知】" + (char)10 + str_AssignNo3,
                                        ConnURL = EMMA_URL,
                                        E_MAIL = v.Agent_Mail,
                                };
                                using (IDbConnection db = DBTool.GetConn())
                    ...
                    foreach (var u in list5)
                    {
                        logger.Info("改 Warr_Mail" + u.PID);
                        Sql = @"UPDATE BusinessData SET Warr_Mail = '1' where PID=@PID ";
                        using (IDbConnection db = DBTool.GetConn())
                            db.Execute(Sql, new { PID = u.PID });    //*/
                    }
                    context.Response.Write(JsonConvert.SerializeObject(new
                    {
                        status = "success",
